...
- **Global scope**: `F:/CLAUDE/ERP_PLANT8_260917/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/CLAUDE/ERP_PLANT8_260917/.claude/notes/`)
...
2. Glob `F:/CLAUDE/ERP_PLANT8_260917/.claude/notes/*.md` (if directory exists) — global notes
...
Global (F:/CLAUDE/ERP_PLANT8_260917/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/CLAUDE/ERP_PLANT8_260917/.claude/notes/` — works in any directory
