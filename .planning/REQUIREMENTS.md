...
- [x] **PROJ-01**: 프로젝트를 등록·목록·상세로 다룬다(클라이언트, 담당 PM, 팀, 기간, 상태)
- [x] **PROJ-02**: 견적 줄 원장에 견적가·실행가(예상 원가)를 입력하면 차익은 서버가 계산한다. 브라우저 계산값은 저장하지 않는다. 지출결의·구매 요청 등 연결 문서가 있는 견적 줄은 보관(삭제)하지 않고 '취소' 상태(견적가 0, 이력 유지)로만 바꾼다
- [x] **PROJ-03**: 「계약 금액」은 입력 칸이 아니라 고객 승인이 표시된 현재 차수의 견적 합계(공급가)이고, 세금계산서 발행일·발행액과 입금일·입금액을 기록한다. 발행액은 공급가액으로 적고 부가세·합계는 자동 계산되며, 입금액은 통장에 들어온 금액(합계)으로 적고 시스템이 공급가액으로 역산한다. 별도 매출 모듈은 없다(D-84)
- [ ] **PROJ-04**: 프로젝트 상태는 수주중 → 진행 → 정산 → 완료와 미수주 다섯 가지다(D-75). 수주중은 제안·PT 단계이고 그때 쌓인 비용은 진행으로 바뀌면 그대로 프로젝트 비용이 되며, 미수주로 닫으면 팀 손익의 미수주 비용이 된다(PNL-07). 진행 → 정산은 종료일 다음 날 00:00(KST)에 시스템이 바꾼다(D-76). 사람의 전환은 수주중 → 진행(시작일 필요, 종료일이 비면 시작일로, D-82)·수주중 → 미수주·미수주 → 진행 복귀(팀장 이상 — 팀장은 자기 팀, 사용자 D11), 정산 → 완료(Phase 4는 대표·시스템 관리자 직접, Phase 5부터 PM 결재 요청 → 대표 승인, D-79), 정산 → 진행은 팀장 이상이 종료일을 늦출 때(D-80)다. 정산에서 PM은 실행가를 고치고 견적가 0인 새 줄을 더할 수 있다(D-78, 사용자 D10·D12). 잠기는 상태는 완료 하나뿐이고 완료된 프로젝트에만 새 지출결의를 올릴 수 없다
- [x] **PROJ-05**: 이전 프로젝트·견적 줄을 복사해 새로 만들 수 있고, 반복 입력 칸은 기본값이 채워진다
- [ ] **PROJ-06**: 완료(정산) 처리 시 시스템이 미결 지출결의·미매칭 견적 줄·매출 미입력을 점검해 이유와 함께 막는다. 강행 허용 여부는 설정
- [x] **PROJ-07**: 견적은 사전 견적 → 상세 견적 차수(1차·2차…)로 관리되고, 차수마다 "고객 승인" 표시를 둔다. 승인 전에는 그 차수의 줄에서 지출결의·구매 요청을 올릴 수 없다(설정으로 끌 수 있음)
...
- [ ] **PNL-06**: 완료(정산) 처리된 프로젝트의 손익은 스냅샷으로 고정되고, 이후 설정 변경에 소급되지 않는다. 완료 뒤 계약 금액·매출이 정정되면 스냅샷은 재계산하지 않고 차이만 표시한다. 완료(정산)는 정산 결재 문서(PM 기안 → 대표 승인, D-98)로 처리되며 대표 승인 순간 스냅샷이 찍힌다
...
- [x] **UX-05**: 모든 데이터 입력을 키보드만으로 끝낼 수 있고, 표 입력(견적 줄·법인카드·목표 등)은 엑셀과 같은 키 구성을 따른다: Tab/Enter·방향키로 칸 이동, 여러 칸 복사·붙여넣기, Esc 취소, 단축키로 저장·새 줄. 표의 일괄 저장은 전부 저장 또는 전부 거부이며 충돌·오류 칸을 표시한다. 자체 구현/라이브러리 선택은 Phase 4 계획에서 정한다
