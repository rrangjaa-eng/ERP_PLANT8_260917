...
- [ ] **MAST-02**: 직원 등록 = 사람 + 계급 + 팀 선택. 입사자 추가에 코드 수정이 없다. 팀 소속은 발령일 이력으로 남고, 비용 귀속·팀 손익은 사용일 시점의 소속 팀을 따른다
...
- [ ] **EXP-03**: 결재는 기본 4단(팀장→본부 책임자→경영관리→대표), 빈 자리는 건너뛴다. 승인·반려(기안자에게 복귀)·회수가 된다. 상태 문서는 version 컬럼 낙관적 잠금으로 승인↔회수·승인↔반려 동시 조작을 막는다
- [ ] **EXP-04**: 결재선은 제출 시점에 문서에 고정된다. 이후 설정 변경이 진행 중 문서에 영향을 주지 않는다. 고정되는 것은 단계별 계급이며, 사람은 표시 시점의 현재 담당으로 해석하고 이미 승인한 사람은 그대로 보존된다
...
- [ ] **PNL-02**: 줄마다 가장 굳은 값 하나만 비용으로 든다: 증빙액 > 지출결의 승인액 > 견적 실행가. 진행 중에는 예상값이 든다. 회사 대납 세금(EXP-15)과 견적 외 비용(EXP-16)도 그 프로젝트의 비용에 든다. 목록·Excel·팀 합계는 우선순위 CASE 집계 1회로 계산하고 상세는 근거(trace)를 함께 만든다. '목록 합계 = 상세 합계' 테스트가 있다
...
- [ ] **MIG-01**: 인트라넷 데이터를 옛 표 구조 그대로 가져오지 않고, 새 시스템의 표·코드표·상태·번호 서식에 맞게 변환해 입력한다. 원본 행마다 새 시스템의 대응 행이 있음을 검증한다(멱등·재실행). 적재와 검증은 단일 트랜잭션이며 검증 실패 시 ROLLBACK하고 migration_runs에 기록한다. 이전 행은 source='intranet'으로 표시된다. 추출·변환은 Phase 4부터 시작해 실제 데이터를 픽스처로 쓴다
...
- [ ] **MIG-05**: 리허설을 반복한 뒤 전환일에 인트라넷을 조회 전용으로 바꾸고 새 시스템만 입력한다. 리허설은 source='intranet' 행만 삭제·재적재하고 설정·권한은 보존하며 demo 행은 전환일에 삭제한다. 전환 착수 조건은 회사 GCP에서 배포 스크립트 1회 성공이고, 전환일에는 그 사이 인트라넷에 쌓인 델타만 이전한다
...
*Last updated: 2026-09-17 after /plan-ceo-review (23 findings + D3·D4·D5 + OV-1..8 folded in; OPS-06·OPS-07 added; OV-3 redefined → EXP-15 증빙 종류별 세금 자동 계산; CERT-01 QR 진입; EXP-16 경영관리 카드 사용 대리 등록·개인 비용 팀 귀속·경품 대납 세금 비용 포함; 89/89 mapped to Phases 1-10)*
