...
- [x] **PROJ-03**: 프로젝트에 매출 최소 칸(계약 금액, 세금계산서 발행일·발행액, 입금일·입금액)을 기록한다. 계약 금액·발행액은 공급가액으로 적고 부가세·합계는 자동 계산된다. 입금액은 통장에 들어온 금액(합계)으로 적고 시스템이 공급가액으로 역산한다. 별도 매출 모듈은 없다
