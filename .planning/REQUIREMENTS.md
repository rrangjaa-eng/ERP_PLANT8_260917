...
- [ ] **PROJ-04**: 프로젝트 상태를 수주중 → 진행 → 완료(정산)로 바꿀 수 있고, 수주하지 못한 건은 미수주로 닫는다. 완료된 프로젝트에는 새 지출결의를 올릴 수 없다. 수주중일 때 쌓인 비용은 진행으로 바뀌면 그대로 그 프로젝트의 비용이 되고, 미수주로 닫히면 팀 손익의 미수주 비용으로 내려간다. 뒤늦게 수주하면 미수주 → 진행으로 되살릴 수 있다
...
- [ ] **EXP-08**: 수주 단계 비용(제안·PT 등)은 수주중 프로젝트에 붙이고, 프로젝트가 아예 열리지 않은 건만 프로젝트 없이 팀 이름으로 올린다(팀 관리비와 종류를 구분한다). 직원 개인이 쓴 비용(개인 카드 사용·개인 지출결의)이 프로젝트에 연결되지 않으면 사용일 기준 그 직원의 소속 팀 비용으로 귀속되고, 팀장 화면에 '프로젝트 미연결'로 표시된다
...
- [ ] **PNL-07**: 팀 손익 = 팀 프로젝트 손익 합 − 미수주 비용(미수주로 닫힌 프로젝트의 비용 합) − 팀 직접 관리비(팀 이름 지출 + 프로젝트 미연결 직원 개인 비용, 사용일 시점 소속 기준). 팀장은 자기 팀, 대표·경영관리는 전체를 본다
