...
- [ ] **PROJ-04**: 프로젝트 상태는 수주중 → 진행 → 완료(정산)와 미수주 네 가지다. 수주중은 제안·PT 단계이고 그때 쌓인 비용은 진행으로 바뀌면 그대로 프로젝트 비용이 되며, 미수주로 닫으면 팀 손익의 미수주 비용이 된다(PNL-07). 전환은 수주중 → 진행, 수주중 → 미수주, 진행 → 완료(정산), 미수주 → 진행 복귀이고, 잠기는 상태는 완료(정산) 하나뿐이라 완료된 프로젝트에만 새 지출결의를 올릴 수 없다
...
- [ ] **PNL-07**: 팀 손익 = 팀 프로젝트 손익 합 − 미수주 비용 − 팀 직접 관리비. 미수주 비용은 미수주로 닫힌 프로젝트(PROJ-04)에 쌓인 비용의 합이며, 프로젝트가 열리지 않아 팀 이름으로 올라온 '미수주 비용' 종류 지출(EXP-08)을 더한다. 팀 직접 관리비는 '팀 관리비' 종류의 팀 이름 지출 + 프로젝트 미연결 직원 개인 비용(사용일 시점 소속 기준)이다. 팀장은 자기 팀, 대표·경영관리는 전체를 본다
