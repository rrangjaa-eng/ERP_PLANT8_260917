...
- **D-05:** 승격 흐름: main 병합 → CI → 스테이징 자동 배포(migrate Job → 0% 리비전 → 스모크 → 100%) → 사용자가 스테이징에서 확인 → GitHub Actions의 프로덕션 워크플로를 사용자가 **수동 실행**(`workflow_dispatch`, GitHub 화면의 "Run workflow" 버튼; 입력은 배포할 git SHA, 기본값은 현재 스테이징에 배포된 SHA) → 같은 git SHA 이미지를 프로덕션에 같은 순서로 배포. 빌드는 한 번, 태그 규칙 없음, 실행 기록은 GitHub Actions 로그에 남는다. GitHub Environments·required reviewers는 쓰지 않는다 — 저장소는 개인 계정 `rrangjaa-eng`의 비공개 **무료** 저장소로 유지하며(사용자 결정 C, 2026-09-18; 같은 날의 B를 대체) 무료 플랜의 비공개 저장소에는 Environment 기능이 없다. 변수·시크릿은 저장소 수준(Settings → Secrets and variables → Actions)에 둔다. 프로덕션 잡은 실행 전에 지정 SHA의 이미지가 존재하고 스테이징에 배포돼 있는지 확인하고 아니면 중단한다. 회사 GitHub 조직으로의 이전은 이월한다 — 이전하면 WIF가 저장소 경로에 묶여 있어 `bootstrap-gcp.sh`를 다시 돌려야 한다.
...
- 사용자는 배포마다 자기 손이 가는 방식을 원하지 않는다 — "배포된다"의 증명은 GitHub에서 "Run workflow" 버튼 하나로 끝나야 한다(D-05 C). 저장소 기본 브랜치는 `main`으로 맞춘다(현재 `master` → PR #5 병합 전에 GitHub에서 이름 변경)
...
- 저장소를 회사 GitHub 조직으로 이전 — 코드 소유권을 회사로. 이전 시 WIF 속성 조건(저장소 경로)·저장소 변수·Claude GitHub 앱 설치를 다시 해야 하므로 `bootstrap-gcp.sh` 재실행 포함
