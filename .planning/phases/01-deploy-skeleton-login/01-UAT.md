...
expected: |
  로컬에서 돌던 서버·DB를 전부 내리고 임시 상태(컨테이너, 캐시, 락 파일)를 지운 뒤 `pnpm db:dev` → `pnpm dev`로 맨바닥에서 올린다. 서버가 오류 없이 뜨고 마이그레이션·시드가 끝나며 `/login`이 뜨고 로그인이 된다.
