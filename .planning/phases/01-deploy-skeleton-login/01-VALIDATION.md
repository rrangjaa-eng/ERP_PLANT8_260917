...
| 1-01-03 | 01 | 1 | AUTH-02, AUTH-04, OPS-04 | T-1-02 / T-1-03 | sign-up 4xx, env 오류 메시지에 값 없음, healthz 최소 노출 + domain 경유(app→db 직접 import 없음), 로그에 비밀번호 없음, sliding 쿠키 재발급(get-session Set-Cookie) | unit + integration | `pnpm vitest run --project unit test/unit/env.test.ts test/unit/log.test.ts && pnpm vitest run --project integration test/integration/auth.test.ts test/integration/healthz.test.ts` | ❌ W0 | ⬜ pending |
...
| 1-02-02 | 02 | 2 | AUTH-01 | T-1-06 / T-1-07 / T-1-08 | 5회 실패 잠금(DB), 15분 창, 관리자 해제, IP 429(x-forwarded-for 마지막 항목 → proxy.ts x-client-ip, 헤더 부재 fail-closed), 계정 존재 비노출 | unit + integration | `pnpm db:migrate && pnpm vitest run --project unit test/unit/lockout.test.ts test/unit/client-ip.test.ts && pnpm vitest run --project integration test/integration/lockout.test.ts test/integration/rate-limit.test.ts test/integration/accounts.test.ts` | ❌ W0 | ⬜ pending |
...
| 1-06-01 | 06 | 5 | OPS-01, OPS-02 | T-1-31 / T-1-34 / T-1-35 / T-1-27 | 더티 트리 거부, set -x 없음, 스모크(GET 2 + Origin POST) 실패 시 트래픽 불변, 경보 upsert가 승격 앞, 공인 IP 없음, Job 3개 env 계약(BETTER_AUTH_SECRET·URL) + DB_ADMIN_PASSWORD는 db-bootstrap만, account Job `--command=node,dist/cli/account-cli.mjs`, --domain no-op, prod는 빌드 거부(ProdImageMissing, D-05), 실패 단계명 출력, rollback은 서빙 리비전 기준 | unit (fake gcloud) | `bash -n scripts/deploy.sh && bash -n scripts/rollback.sh && pnpm vitest run --project unit test/unit/deploy/deploy-sh.test.ts test/unit/deploy/rollback-sh.test.ts` | ❌ W0 | ⬜ pending |
| 1-06-02 | 06 | 5 | OPS-01 | T-1-29 / T-1-30 / T-1-32 / T-1-33 / T-1-45 | WIF attribute-condition 리포 한정, 입력을 env로 전달, id-token 최소 권한, deploy 스텝 `shell: bash`(pipefail — 실패한 스테이징이 success로 보이지 않음), production은 workflow_dispatch 전용 + 스테이징 SHA 가드 스텝, 세 워크플로에 `environment:` 키 없음(D-05) | unit (fake gcloud + text) | `bash -n scripts/bootstrap-gcp.sh && pnpm vitest run --project unit test/unit/deploy/bootstrap-sh.test.ts test/unit/deploy/workflows.test.ts test/unit/ci-guard.test.ts` | ❌ W0 | ⬜ pending |
...
| 1-07-03 | 07 | 6 | OPS-01, OPS-06 | T-1-39 / T-1-40 | 배포 URL healthz·login 응답, 상태 화면 권한 실측, 위조 XFF 11회 → 429 실측 | smoke + human-check | `curl -fsS "$SERVICE_URL/healthz" \| grep -q '"ok":true' && curl -s -o /dev/null -w '%{http_code}' "$SERVICE_URL/login" \| grep -q '^200$' && test -s .planning/phases/01-deploy-skeleton-login/01-07-DEPLOY-LOG.md` | n/a (배포 후) | ⬜ pending |
| 1-08-01 | 08 | 7 | OPS-01 | T-1-41 / T-1-43 / T-1-45 | production 수동 실행(workflow_dispatch target=production) + 가드 스텝, 실행 로그가 기록(D-05) | checkpoint:human-action | — (GitHub Run workflow) | — | ⬜ pending |
...
- [ ] `scripts/dev-db.sh` — 로컬/클라우드 Postgres 16 `erp`·`erp_test` (Plan 01-01 Task 2, D-01)
- [ ] `.claude/settings.json` SessionStart 훅 + `scripts/install_pkgs.sh` 확장(pnpm install·Playwright 링크) — 클라우드 세션 자동 기동 (Plan 01-01 Task 3, D-01)
- [ ] `lib/log.ts` + `test/unit/log.test.ts` — JSON 로그 계약(이후 플랜의 `auth.*`·`db.*` 이벤트 단언이 의존) (Plan 01-01 Task 3)
...
| production 워크플로 실행 권한이 본인뿐인지(write 협업자 목록) | OPS-01 (D-05, T-1-45) | 협업자 목록은 GitHub 설정 UI/API로만 확인 — GitHub Environments는 쓰지 않는다 | 01-07 Task 2 단계 6(설정 UI) + verification의 `gh api …/collaborators` |
