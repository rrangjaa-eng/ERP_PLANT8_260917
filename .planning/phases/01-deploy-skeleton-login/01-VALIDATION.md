...
| 1-01-03 | 01 | 1 | AUTH-02, AUTH-04, OPS-04 | T-1-02 / T-1-03 | sign-up 4xx, env 오류 메시지에 값 없음, healthz 최소 노출 + domain 경유(app→db 직접 import 없음), 로그에 비밀번호 없음 | unit + integration | `pnpm vitest run --project unit test/unit/env.test.ts test/unit/log.test.ts && pnpm vitest run --project integration test/integration/auth.test.ts test/integration/healthz.test.ts` | ❌ W0 | ⬜ pending |
...
| 1-06-01 | 06 | 5 | OPS-01, OPS-02 | T-1-31 / T-1-34 / T-1-35 / T-1-27 | 더티 트리 거부, set -x 없음, 스모크 실패 시 트래픽 불변, 공인 IP 없음, Job 3개 env 계약(BETTER_AUTH_SECRET·URL) + DB_ADMIN_PASSWORD는 db-bootstrap만, --domain no-op, prod는 빌드 거부(ProdImageMissing, D-05) | unit (fake gcloud) | `bash -n scripts/deploy.sh && bash -n scripts/rollback.sh && pnpm vitest run --project unit test/unit/deploy/deploy-sh.test.ts test/unit/deploy/rollback-sh.test.ts` | ❌ W0 | ⬜ pending |
