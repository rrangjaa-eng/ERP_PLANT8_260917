...
| 1-06-01 | 06 | 5 | OPS-01, OPS-02 | T-1-31 / T-1-34 / T-1-35 / T-1-27 | 더티 트리 거부, set -x 없음, 스모크 실패 시 트래픽 불변, 공인 IP 없음, Job 3개 env 계약(BETTER_AUTH_SECRET·URL) + DB_ADMIN_PASSWORD는 db-bootstrap만, --domain no-op | unit (fake gcloud) | `bash -n scripts/deploy.sh && bash -n scripts/rollback.sh && pnpm vitest run --project unit test/unit/deploy/deploy-sh.test.ts test/unit/deploy/rollback-sh.test.ts` | ❌ W0 | ⬜ pending |
| 1-06-02 | 06 | 5 | OPS-01 | T-1-29 / T-1-30 / T-1-32 / T-1-33 | WIF attribute-condition 리포 한정, 입력을 env로 전달, id-token 최소 권한, deploy 스텝 `shell: bash`(pipefail — 실패한 스테이징이 production 승인 대기로 가지 않음) | unit (fake gcloud + text) | `bash -n scripts/bootstrap-gcp.sh && pnpm vitest run --project unit test/unit/deploy/bootstrap-sh.test.ts test/unit/deploy/workflows.test.ts test/unit/ci-guard.test.ts` | ❌ W0 | ⬜ pending |
