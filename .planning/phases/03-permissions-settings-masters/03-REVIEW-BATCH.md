...
| 7 | 03-07 | ✓ 완료 (프로덕션 전용 결함 1건 수정 중) | `4199040` | ✓ | ✓ | ✓ 509 | ✓ 691 | ⚠ 85/86 |
...
## 사용자 판단 기록 (2026-09-21)

실행 중 열린 항목에 대한 사용자 결정이다. 다음 사람이 같은 것을 다시 조사하지 않도록 남긴다.

### 설정 라벨 줄바꿈 — **그대로 둔다**

`/admin/settings`에서 "원천징수 절사 단위(원)"가 PC 1440px에서도 §6-3의 96px 고정 라벨 칸 안에서 두 줄로 꺾인다(`screens/wave-4/settings-pc.png`에서 확인 가능).

**결정: 손대지 않는다.** 근거 — 관리자 전용 화면이고, 기능을 해치지 않으며, §6-3을 고치면 지출결의 폼을 포함한 **모든 폼 화면**에 반경이 간다. CLAUDE.md의 "화면 하나만 예외 금지" 때문에 설정 화면만 넓히는 것도 불가하다.

`SYSTEM.md`는 수정하지 않았다 — 계약 변경이 아니라 현상 수용이다.

### e2e 워커 격리 — **Phase 4 첫 작업으로 미룬다**

플레이크 3계열(`keyboard-nav` · `permissions-grid` · `corp-cards`/`mobile-*`)의 원인은 단일 `erp_test` DB 공유 + 워커별 초기화 부재로 특정됐다.

**결정: Phase 3에서 고치지 않는다.** 근거 — 아무것도 막지 않고(재실행·CI 통과), 어느 플랜에도 없는 테스트 인프라 작업이라 지금 하면 Phase 3 범위를 실행 중 판단으로 넓히게 되며, `playwright.config.ts`의 `webServer.env` 블록은 02-07이 불변으로 못박아 계획 단계 재검토가 필요하다.

**다만 무한정 미루지 않는다 — Phase 4의 첫 작업으로 잡는다.** 비용이 스펙 수에 비례해 커지고(현재 84개+), Phase 4가 스펙을 추가하기 **전**에 고치는 것이 가장 싸다. 작업 카드로 등록해 뒀다.

### 웨이브 4 TDD — **전면 재작성하지 않고 급소만 적대적으로 검증했다**

결과:

- **`effective_from <= asOf` 경계 포함** → ✓ 이미 충분히 덮여 있다. 구현은 `lte`이고, 단위·통합 **두 층**에서 "적용 시작일이 조회 기준일과 정확히 같은 행이 유효값이다"를 검사하며, 보수 케이스("하루 뒤 행은 아직 유효하지 않다")까지 있다. 구현을 베껴 쓴 테스트에서 나오기 어려운 형태다
- **`readBy` 예외의 폭** → 아래 별도 항목 참조


## 웨이브 7 — 03-07 행동 로그 · 보관함 · CSV 내보내기

**소요 145분**(최장) · 요구사항 ADMN-10, ADMN-12, OPS-05 · 커밋 13건

### 게이트 (실행자 보고, 로컬 dev 기준)

typecheck · lint · `lint:sql`(9파일 0 issues) · build(`/admin/action-log`·`/admin/archive` 생성) PASS. unit **509** · integration **691**(신규 action-log-query 10 + archive 8) · e2e **86/86**(2회차). `package.json` 신규 의존성 **0** — 체크포인트 ④ 유지.

오케스트레이터 독립 확인: 테스트 약화 0건 · `.squawk.toml`·eslint 무변경 · 루트 오염 없음 · UTF-8 BOM 구현 확인(`domain/action-log/export.ts`) · `action-log.export`가 `EXPORT_REGISTRY`에 `dtoName: "ActionLogDto"`로 등록돼 누수 스캔이 계급별로 검사한다.

### 체크포인트 ④ 확정

UTF-8 BOM CSV · 신규 의존성 0. 반환 형태를 직렬화와 분리해 Phase 9에서 `.xlsx`로 승격할 때 함수 교체만으로 되게 했다.

### ⚠ CI 실패 — 프로덕션 빌드 전용 결함 (수정 중)

**로컬 dev 86/86 통과, CI 85/86 실패.** `archive.spec.ts:53`에서 복원 토스트(`복원 · … 복원됨`)가 15초 안에 `element(s) not found`다.

**플레이크가 아닌 근거**: CI 3회 연속 같은 단언 · 최근 실행은 **새 DB**(공유 오염 아님) · "숨겨짐"이 아니라 **아예 렌더되지 않음** · 나머지 85개는 통과.

**원인 방향**: `playwright.config.ts`가 `process.env.CI ? "pnpm build && pnpm start" : "pnpm dev"`다. **CI는 프로덕션 빌드, 로컬은 dev.** 실행자가 dev에서만 검증해 타이밍 플레이크로 오판했고, 그 판단이 이 건을 50분 잘못된 방향으로 끌었다.

**영향**: staging·production도 프로덕션 빌드다 → **실사용에서 보관함 복원 시 성공 여부를 알 수 없다.** SYSTEM.md §7-6 토스트 계약 위반. 배포 전 수정이 필요하다.

`CI=true`로 재현 후 원인 규명하도록 수정 중이다. **단언을 고치지 않는다**(타임아웃 확대·locator 완화·skip 금지) — 그렇게 녹색을 만들면 깨진 화면이 배포된다.

### 실행자가 판단한 것

1. `domain/action-log/filter-keys.ts`를 **의존성 없는 말단 모듈로 분리** — 도메인 인덱스 전체를 클라이언트 컴포넌트가 import하면 `tls`/`util` 모듈 오류로 빌드가 깨진다
2. `pruneActionLogRows` → `markActionLogRowsPruned` 개명 — 플랜 자체의 검증 스크립트가 import 줄과 부분 문자열 충돌로 거짓 음성을 냈다
3. **보관해도 관리자 목록에서 사라지지 않는다는 것을 `scopeFor`의 `includeArchived` 계약에서 발견** — 「보관됨」 상태 태그를 붙이고 동작 버튼을 숨기는 처리를 다섯 화면(코드표·법인카드·거래처·계급·조직)에 추가했다. 03-05가 사람 화면에 세운 선례를 따른 것이고, 없으면 기능이 읽히지 않는다
4. `archive.spec.ts`의 "복원 후 보관함이 전역으로 빈다" 단언을 **"내 항목의 행이 사라진다"로 좁혔다** — `permissions-grid.spec.ts`가 임시 계급 26개를 정리 수단으로 보관 처리하고 복원하지 않아, 공유 DB에서 보관함이 진짜로 빌 수 없다. 테스트 약화가 아니라 **단언을 정확하게 만든 것**이다

### 귀속 정정

실행자가 `deploy.sh`의 48→32바이트 키 버그를 자기 발견으로 보고했으나, **그 건은 오케스트레이터가 찾아 별도 에이전트로 고쳤다**(`e9e64eb`·`05ec647`). 실행자 작업 사이에 커밋이 끼어 자기 것으로 집계됐다. 내용은 정확하고 귀속만 틀렸다.

---

## 공정 교훈 — DOM 감사를 게이트 **앞**으로 옮겨야 한다

### 이번 페이즈에서 실제로 돈 순서

```
구현 → 전체 게이트(10~20분) → 실행자 「완료」 → DOM 감사 → 결함 발견 → 수정 → 전체 게이트 다시
```

**게이트를 두 번 돌렸다.** 통합 691개 + e2e 86개가 매번 10~20분이고, 웨이브 5·7은 세 번 이상 돌았다.

### 왜 순서를 바꿔도 안전한가

이번 페이즈에서 DOM 감사가 잡은 **결함 10건이 전부 게이트가 녹색인 상태에서 나왔다.** 게이트가 잡을 수 있는 종류였다면 순서와 무관하게 게이트가 먼저 잡았을 것이다. 즉 **순서를 바꿔도 놓치는 것이 없고, 게이트 한 번을 아낀다.**

| # | 결함 | 웨이브 |
|---|---|---|
| 1 | 격자 2단 sticky 머리글이 세로 스크롤에서 무효 | 3 |
| 2 | 머리글 두 행 겹침(1번을 고치자 드러남) | 3 |
| 3 | 원본 Zod 오류 JSON 노출 | 4 |
| 4 | 원본 SQL INSERT + 내부 user id 노출 | 5 |
| 5 | org·roles 375px 가로 스크롤 | 5 |
| 6 | 인라인 입력 9개에 접근 가능한 이름 없음 | 5 |
| 7 | 3차 버튼 형태 붕괴 | 5 |
| 8 | CSV BOM이 파일에 도달하지 않음 | 6·7 |
| 9 | 표 안 3차 버튼 20px (공유 컴포넌트) | 6·7 |
| 10 | 거래처·행동 로그 375px 가로 스크롤 | 6·7 |

### 바꿀 순서

```
실행자      구현 → 싼 게이트(lint·typecheck·build, 약 2분)
   ↓
독립 감사자  DOM 실측 판정  ← 반드시 CI=true (프로덕션 빌드)
   ↓
실행자      결함 수정
   ↓
실행자      전체 게이트 한 번 → SUMMARY
```

DOM 감사는 화면이 렌더돼야 하므로 구현 앞으로는 못 간다. 하지만 **비싼 게이트 앞으로는 갈 수 있고**, 그게 절약의 전부다.

### 두 가지 조건

**① 감사자는 실행자와 분리되어야 한다.** 이번에 결함이 잡힌 건 감사자가 독립적이었기 때문이다. 웨이브 7 실행자는 dev에서 e2e 86/86을 보고 CI 실패를 「플레이크」로 결론냈고, 독립 감사가 프로덕션 빌드 차이를 짚었다. 자기 화면을 자기가 감사하면 구현 뒤에 쓴 테스트가 구현을 따라 쓰는 것과 같은 편향이 생긴다.

**② 감사는 프로덕션 빌드에서 해야 한다.** `playwright.config.ts`가 `process.env.CI ? "pnpm build && pnpm start" : "pnpm dev"`다. 이 페이즈에서 **dev에서만 통과하는 결함이 두 번** 나왔다:

- 암호화 키가 e2e 경로에 공급되지 않았는데 gitignore된 `.env.local`이 로컬에서 덮어 줬다
- 보관함 복원 토스트가 `revalidatePath`로 사라지는 `<tr>` 안에 렌더돼, 성공 9ms 뒤 행과 함께 제거된다. dev의 느린 컴파일이 그 창을 넓혀 가려줬다

감사 지시에 `CI=true`를 넣지 않은 것은 오케스트레이터의 누락이다. 그래서 토스트 결함이 감사를 통과하고 CI에서 세 번 터진 뒤에야 잡혔다.

### Phase 4에 반영할 것

- 플랜의 검증 단계를 「싼 게이트 → 독립 DOM 감사 → 수정 → 전체 게이트」로 쓴다
- 감사 지시에 **`CI=true` 프로덕션 빌드**를 명시한다
- 「로컬 dev 통과」를 완료 신호로 쓰지 않는다
