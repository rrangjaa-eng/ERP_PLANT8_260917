...
**5. [계획·코드 충돌 — 기록] (d) 동시 변경 거부는 서버 쪽에서 상태를 바꿔 재현**
- 두 번째 브라우저 대신 DB로 상태를 먼저 바꾼 뒤 모달에서 제출해 서버 문자열 `상태가 {라벨}로 바뀜 · 새로 고침`을 확인.
...
- 전환 성공으로 트리거가 사라졌을 때 포커스를 h1로 돌리는 폴백은 실효가 약함(h1에 tabIndex 없음 · 새로 고침 전에 모달이 닫힘)
