...
- `CI=true pnpm test`는 돌리지 않았다(아래 「화면 검증」)

## 화면 검증

- 싼 게이트(lint · typecheck · build)와 각 태스크 `<verify>`의 표적 Playwright 스펙은 위와 같이 통과
- **독립 DOM 감사(Task 3 ④)와 `CI=true pnpm test` 전체 게이트는 대기 중** — 실행자가 아닌 별도 에이전트가 `CI=true`로 실측해야 해서 오케스트레이터가 이 플랜 뒤에 돌리고, 판정(1280 · 1024 · 375 가로 스크롤 · 폰 P1 세 열 · 1000 · 375 구조 컨트롤 부재 · 조정 행 Enter·클릭 무편집)과 폭별 실측값은 뒤 수정 커밋이 이 절에 적는다
- 감사에 넘길 관찰점: 추가 버튼 줄이 이제 flex + `--s-3` 간격(버튼 셋까지) · 상한에서는 버튼 하나 + 이유 한 줄 · 합계 행 muted `조정 줄 N칸 건너뜀` · 소분류 편집 중 설명 줄(편집 중에만 행 높이가 늘어남)
