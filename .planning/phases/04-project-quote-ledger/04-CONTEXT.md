...
- **D-63:** **견적가 = 수량(기본 1) × 단가, 서버 계산·저장.** 실행가는 금액 한 칸. 차익 = 견적가 − 실행가도 서버 계산. 브라우저 계산값은 저장되지 않는다(PROJ-02). 이전 데이터는 수량 1·단가 = 옛 금액으로 변환.
  - **2026-09-23 대체(마지막 문장만):** 「이전 데이터는 수량 1·단가 = 옛 금액으로 변환」은 데이터 이전 없음(2026-09-23 사용자 결정)으로 적용되지 않는다. 견적가 = 수량 × 단가 서버 계산은 그대로다.
...
- **D-72:** **extract의 입력은 MySQL 덤프 파일이다.** 리포 밖 경로를 환경 변수(`INTRANET_DUMP_PATH`)로 받는다. 네트워크·계정 불필요, 재실행 결정적. 인트라넷 RDS 직접 접속은 하지 않는다(고정 IP 없음, 드라이버 의존성). Phase 8 델타 이전은 새 덤프로.
  - **2026-09-23 대체:** 데이터 이전 없음(2026-09-23 사용자 결정) — 04-03 철회, 커밋된 추출 스크립트는 `6b7519f`로 되돌림. 이 결정은 더 이상 적용되지 않는다.
...
- **04-03(인트라넷 덤프 변환) 보류** — 덤프 파일을 다음 세션에서 제공. 보류하는 동안 ROADMAP Phase 4 성공 기준 7은 미착수다 **→ 2026-09-23 철회: 데이터 이전 없음(2026-09-23 사용자 결정). 04-03 플랜을 지웠고 ROADMAP 성공 기준 7은 철회로 표시했다.**
