...
must_haves:
  truths:
    ...
    - "누수 스캔이 커스텀 칸 항목을 포함한다 — 활성 거래처 칸 정의 × 시드 계급마다, 그 계급에 `vendor.value`를 켠 상태로 끄기 전 값이 직렬화에 있음을 먼저 확인한 뒤, 그 칸을 끈 계급의 거래처 DTO 직렬화에 그 칸의 값이 없다(헛통과 없음 · ROADMAP 기준 4·5)"
  ...
  key_links:
    ...
    - from: "domain/custom-fields/visibility.ts"
      ...
      via: "`grantCustomFieldsToRole`이 `withTransaction` 첫 문장으로 01의 `lockCustomFieldGrants(viewer, tx)` → `listFieldDefinitions(…, tx)`(같은 연결) → `insertVisibilityIfAbsent(…, tx)`"
...
**공유 테스트 환경:** 같은 wave(4)의 02와 공유 `erp_test` DB·E2E 포트 3100·`next build` 디렉터리를 같이 쓴다. 그래서 통합 테스트·`pnpm build`·E2E 명령은 모두 `flock /tmp/plant8-erp-test.lock `을 앞에 붙여 한 번에 하나씩 돈다(단위·lint·typecheck는 잠금 없음). `pnpm db:dev`는 로컬 Postgres를 띄울 뿐 DB를 다시 만들지 않는다. 통합 DB는 vitest `test/integration/global-setup.ts`(마이그레이션)와 `test/integration/setup.ts`(매 테스트 TRUNCATE → `seedMasterData`)가, E2E DB는 `test/e2e/global-setup.ts`(12–30행: 스키마 비움 → 마이그레이션 → `seedMasterData`)가 준비한다. 테스트는 그 위에 자기 픽스처를 넣는다. 이 플랜은 DB를 따로 준비하지 않는다.
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - (통합, 누수 스캔) 결정적 생성기가 (활성 칸 × 시드 계급) 케이스를 만들고, 각 케이스에서 그 계급에 `vendor.value` 보임 행을 켠 상태로 **끄기 전** 거래처 DTO 직렬화에 두 칸 값이 **있음**을 먼저 단언한 뒤, 그 칸을 끄면 그 칸의 값 문자열이 **없고** 다른 칸 값은 남는다(`vendor.value`가 꺼진 계급은 DTO 전체가 비어 헛되이 통과하므로 사전 양성 단언으로 막는다)
...
④ `test/integration/leak-scan.test.ts` **끝에** `describe("커스텀 칸 축 — …")` 하나: 테스트 안에서 시스템 관리자로 칸 둘과 거래처 하나(두 칸 값은 서로 다른 고유 문자열)를 만들고, 순수 생성기 `buildCustomFieldCases(defs, roles)`(칸 × `SEED_ROLES`, 결정적 순서)의 케이스마다 그 계급에 `admin.vendors` 보기와 `vendor.value` 보임 행(`upsertVisibility`)을 켜고, **먼저** `listVendors`의 JSON 직렬화에 두 칸 값 문자열이 모두 있음을 단언한 뒤, 그 칸만 끄고 그 값 문자열만 사라지고 다른 칸 값은 남아 있음을 단언한다. `domain/seed/index.ts`에 `vendor.value` 행을 더하지 않는다(테스트 안에서만 켠다 — Task 1 acceptance의 seed diff 0 유지 · `domain/vendors`의 `VENDOR_DTO_SPEC`은 모든 칸이 `vendor.value`에 묶여 있고 시드는 이 행을 시스템 관리자·기본 계급에만 주므로, 켜지 않으면 나머지 시드 계급 케이스가 상위 차단으로 헛되이 통과한다). 기존 `describe`·import·헬퍼는 고치지 않는다.
...
⑥ `test/integration/custom-fields.test.ts`는 이 플랜에서 고치지 않는다. 이 플랜은 쓰기 경로(`validatedCustomFields`)를 바꾸지 않는다. 그 파일은 DB 행을 직접 읽어 단언하므로 DTO 거르기와 무관하게 초록이다. 노출 행 픽스처와 거부 이유 단언은 쓰기 판정이 노출 행에 묶이는 05 Task 2가 더한다.
  ...
  <acceptance_criteria>
    ...
    - 커스텀 칸 축의 케이스마다 `vendor.value` 보임 행을 켜는 호출이 있고, 칸을 끄는 호출보다 **앞에** 그 값 문자열이 직렬화에 있다는 단언(양성)이, 뒤에 없다는 단언(음성)이 있다 — 시드 계급 5개 × 칸 2개 케이스 전부에서 양성 단언이 실제로 통과한 출력을 SUMMARY에 인용
    - 세 검증 명령이 전부 초록이다
  </acceptance_criteria>
