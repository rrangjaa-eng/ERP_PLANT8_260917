...
| 단계 | 사용자가 하는 일 | 느낌 | 받치는 절 |
|---|---|---|---|
| 1 | 홈 「내 차례」 `[오늘] 지급 — … · 확인 전 1건` → 3차 `지급 대상 보기` | 할 일이 보임 | S19 |
| 2 | S1에서 예정일 그룹을 훑는다. `확인 전`이 흩어져 있으면 `[증빙 전체 ▾]`에서 `확인 전`으로 모은다 | 통제감 | S1 「보기 필터」 |
| 3 | `확인 전` 행 증빙 칸 3차 `증빙 확인` → 확인 모달에서 파일을 보고 `증빙 확인 Ctrl+Enter` | 목록을 떠나지 않음 | S1 「제자리 증빙 확인」 · SP-7 |
| 4 | 모달이 닫히면 그 행이 저절로 골라지고 포커스가 그 행 이체액 셀에 있다 | 다음 할 일이 바로 앞 | 같은 절 |
| 5 | 이체액을 고친다 → 차이 2행 · 사유 칸 | 실수 방지 | S1 「이체액·차이 사유 편집」 |
| 6 | 금액을 고쳐야 하는 건은 번호 링크로 문서 화면(`from=pay`) → 금액 고쳐 확인 → `지급 대상으로`(또는 `다음 확인 전 {번호}`) → 목록의 선택·편집이 그대로 | 잃을 걱정 없음 | S1 「문서 화면 왕복」 |
| 7 | `지급 완료 N Ctrl+Enter` → 모달(지급일 · 합계 · `다른 쪽 N건 포함`) | 확신 | S2 |
| 8 | 결과 `14:02 지급 완료 5건 · 막힘 2건`(스크린 리더도 읽는다) + 막힌 행 아래 이유, 포커스는 첫 막힌 행 | 남은 일이 분명 | S1 「일괄 결과 알림 · 막힌 행 선택」 |
| 9 | 막힌 행의 다음 한 수(`새로 고침` · `이체액 확인`) — 게이트로 막힌 행은 선택이 이미 풀려 있다 | 해결할 수 있음 | 같은 절 |
| 10 | 잘못 처리한 건 → 문서 화면 `지급 취소`(사유) | 되돌릴 수 있음 | S5 |
...
├───────────────────────────────────────────────────────────────────────────────────────────────┤
│ 예정일 지남                                                                                    │ ← 그룹 머리글
│ ☑  26001-0004  아이오닉9 · 무대설치    스테이지원    신한 ****-**-1234  09-12  확인됨    13,640,000  13,636,700  이체 수수료 │ ← 번호 = 식별자형 링크(H-2)
...
금액을 고치지 않는 증빙 확인을 목록을 떠나지 않고 한다. **1차 경로다**(문서 화면 왕복은 아래 보조 경로). O-2 「막기」는 그대로 — 막는 규칙이 아니라 확인하러 가는 길을 줄인다.
...
- **키:** 3차 셀은 `Enter`로 연다(「링크 셀과 편집 표 키」). 모달 안 `Ctrl+Enter` = 확인, `Esc` = 닫기(포커스는 그 3차로 돌아온다 — §7-17).

#### S1 문서 화면 왕복 — `from=pay` · 편집·선택 보관(보조 경로) (rev 9, C-1)

모달로 할 수 없는 일(금액을 고친 확인 · 문서 전체 보기)은 문서 화면에서 하고 목록으로 돌아온다.

- **가는 길:** 번호 링크(1024~1279와 700~1023은 「프로젝트 · 항목」 1행 링크) · 1024 미만의 증빙 칸 3차 → `/expenses/{id}?from=pay&{지금 보기 · 필터 · 쪽}`.
- **문서 화면(S4 · S5):** `from=pay`일 때만 행동 줄 오른쪽에 3차 `지급 대상으로`(받은 쿼리를 그대로 들고 S1로). 그 문서가 `확인됨`이 되면(응답 상태) 같은 자리에 3차 `다음 확인 전 26001-0009`가 하나 더 선다 — 서버가 같은 필터의 다음 `확인 전` 지출결의 번호를 보내고(없으면 렌더하지 않는다), 같은 `from=pay` 쿼리를 이어 든다. 1차는 「상태 → 1차」 표 그대로 하나다.
- **보관(`sessionStorage`, 탭 수명):** 키 = 보기 + 필터(`pay` · 팀 · 증빙 값). 값 = 고른 행 id · 행마다 이체액 · 차이 사유 편집값 · 지금 쪽. 계좌 · 이름 · 금액 계산값은 보관하지 않는다(행 id와 사람이 적은 값만). 선택 · 편집이 바뀔 때마다 쓴다.
- **되살리기:** 같은 탭에서 같은 키로 S1이 다시 열리면 묻지 않고 되살린다(스스로 떠났다 돌아온 이어 하기라 §7-3 (마)의 `복원 / 버림` 줄을 두지 않는다 — CLAUDE.md §7). **보관한 행 id 전부**(지금 쪽 밖 포함)를 합 요청 한 번에 실어 보내고(합계 번호 +1 — 「합계 줄」 규칙), 서버가 행마다 돌려준 `고를 수 있음` · `version` · 지급 총액으로 **지금 고를 수 있는(P4)** 행만 선택 · 편집을 되살린다(r2 F-2 — 다른 쪽에서 고른 행도 살아남고 S2 결과 줄 ③ `다른 쪽 N건 포함`이 센다). 그 밖의 행(처리됨 · 막힘으로 바뀜)의 보관값은 버린다 — 그 행 증빙 칸 글자가 이유를 말한다.
  보관은 `version`을 싣지 않으므로 지금 쪽 밖 행의 `version` · 지급 총액은 이 응답 값으로 처리 요청에 든다(S2 1차는 모달을 연 합계 번호의 응답을 기다리므로 늘 채워져 나간다). 처리(S2) 응답 뒤 선택이 남은 지금 쪽 밖 행도 다음 합 응답 값으로 바꾼다 — 동시성으로 막혀 선택을 유지한 다른 쪽 행이 옛 `version`으로 다시 막히지 않게(지금 쪽은 「쪽(M-1)」 재조회가 바꾼다).
- **지우는 때:** 처리(S2) 응답 뒤 처리된 행 · 필터를 바꿀 때(그 키 통째로 — 「선택 열」의 「필터를 바꾸면 비운다」와 같은 때) · 탭을 닫을 때(`sessionStorage` 수명).
- 이탈 경고는 `beforeunload`(탭 닫기 · 다른 사이트)뿐이다. 앱 안 이동 가드는 두지 않는다 — 보관이 받는다(C-1 ③).

#### S1 일괄 결과 알림 · 막힌 행 선택 (rev 9, DR-6 · H-3 · M-1 · M-2)

- **서버가 행마다 「지금 고를 수 있음」을 다시 보낸다.** S2 처리 응답은 건마다 결과(처리됨 / 막힘 + 이유 문자열)와 함께, 막힌 행마다 응답 순간의 「상태 → 1차」 표로 다시 판정한 `고를 수 있음` 값을 싣는다.
- **게이트로 막혀 이제 고를 수 없는 행**(`증빙 확인 전` · `증빙 없음` · `결재 통과 전` · 그사이 다른 사람이 지급 완료 — P2 · P3 · 지급 뒤): 선택을 풀고 체크박스는 `aria-disabled`, 이유 줄은 남는다. 그 행의 편집 칸은 사라지므로(편집은 P4 행에만) 보관한 편집값도 버린다.
- **여전히 고를 수 있는 행**(동시성 `다른 사람이 14:01에 바꿈 · 새로 고침` · 지급일 세율로 지급 총액 바뀜 — P4 그대로): 선택을 유지한다. 1차 `지급 완료 N`의 N은 고를 수 있는 고른 행만이라 비활성+체크 행이 생기지 않는다.
- **스크린 리더:** 결과 글자 `14:02 지급 완료 5건 · 막힘 2건`은 처음부터 DOM에 있는 빈 `aria-live="polite"` 영역에 쓴다(SYSTEM §10 「저장 결과 `aria-live`」와 같은 결). 막힌 행 이유 줄의 id를 그 행 체크박스의 `aria-describedby`에 더한다(증빙 칸 글자 id와 함께 — 누르기 전에 읽힌다).
- **포커스:** 모달이 닫히면 첫 막힌 행의 체크박스(`aria-disabled`여도 포커스는 된다 — SYSTEM §10), 막힌 행이 없으면 1차 버튼.
- **쪽(M-1):** 처리 뒤 같은 쪽을 다시 받는다. 그 쪽이 비었으면 행이 있는 마지막 쪽으로 간다(모든 쪽이 비면 「Empty — 지급 대상」). 처리된 id는 다른 쪽 선택과 보관에서도 지운다. 이유 줄은 행 id로 기억해 다시 받은 쪽에 그 행이 있으면 붙인다.
- **결과 글자 자리(M-2):** 필터 줄의 필터 다음(왼쪽 묶음 끝)이고 1차는 오른쪽 끝이다 — 시각이 앞(`14:02 …`)이라 1차 라벨 `지급 완료 N`과 붙어 읽히지 않는다.
...
- **새 건의 기본값(rev 9, M-4 — 알 수 있는 값은 미리 채운다, CLAUDE.md §7):** 서버가 채워 보낸다. 모두 바꿀 수 있다.
  ...
  - `연결` = 그 사람의 직전 등록의 연결 종류 + 프로젝트(`견적 줄` · `견적 외 비용`일 때). 견적 줄 자체는 비운다(줄은 건마다 다르다). 직전 프로젝트를 지금 고를 수 없으면(완료 — 대리 등록 권한자의 `견적 외 비용`은 제외) 종류만 남기고 프로젝트를 비운다. 처음 쓰는 사람은 연결을 비운다. 견적 줄 행에서 들어오면(S14) 그 줄이 이긴다.
...
### S21. 차수 승인 증빙(D-56) — 프로젝트 상세 차수 섹션 (04 S5 「차수 열기」 펼침 + §7-10 첨부 영역) — 담당 PM

Phase 4에서 넘어온 D-56(차수 고객 승인 증빙 첨부). **새 컴포넌트 · 새 레이아웃 꼴 없음** — 04 S5 차수 섹션의 읽기 표(동작 칸)와 「차수 열기」 펼침 섹션 규칙, §7-10 파일 행 · 첨부 영역만 쓴다(UA-621). 첨부는 선택이고 어떤 게이트도 유무를 보지 않는다(06-16).

**04 S5 개정(r2 F-3 — 이 절이 Phase 4 S5 계약을 바꾸는 곳 셋. 새 레이아웃 꼴은 없다):**
- ⒜ **승인된 최신 차수 행의 동작 칸에 3차 하나**(`승인 증빙 {N}`). 04 S5 「현재 차수 행에는 동작이 없다」와 zero-one-many 「차수가 하나뿐이면 … 동작 칸이 비어」는 **승인 안 된** 최신 차수(`현재`)에만 남는다 — 승인된 최신 차수 행에는 `quote.amount` 노출자에게 이 3차가 선다(노출 없는 사람에게는 비어 있다). 「차수 열기」가 이전 차수 행에만 붙고 차수가 하나면 표에 없다는 규칙은 그대로다.
- ⒝ **이전 차수 읽기 섹션(「차수 열기」 펼침)에서 승인 증빙 블록만 쓰기를 허용한다** — 쓰기 권리일 때 그 블록의 파일 행 3차 `삭제`와 첨부 영역. 견적 줄 읽기 표(캡션 `상세 견적 {n}차 견적 줄`)는 04 그대로 편집 · 저장 · 구조 컨트롤 · 복원 줄 · 힌트 · `조정` 그룹이 없다.
- ⒞ **라벨 규칙:** 「차수 열기」는 04 그대로 토글(열리면 `차수 닫기`)이고, `승인 증빙 {N}`은 라벨이 그대로이며 열림은 `aria-expanded`만 말한다(개수가 든 라벨이라 바꾸면 개수를 잃는다). 둘은 같은 펼침 자리 · 한 번에 하나 규칙을 나눠 쓴다.
- 04 S5 기존 E2E(04-24의 차수 섹션 스펙) 단언 중 ⒜ · ⒝에 걸리는 것은 06-16이 이 세 문장대로 고친다 — 04 계약의 나머지는 바꾸지 않는다.

```
프로젝트 상세 ─ 차수 섹션(04 S5, ui/table 읽기)
│ 차수      생성일      줄 수   견적 합계     상태   (동작)            │
│ 2차       2026-09-18  6줄     48,000,000    승인   승인 증빙 1        │ ← 3차 <button aria-expanded aria-controls>
│ 1차       2026-09-12  5줄     45,000,000           차수 열기          │
╞═ 상세 견적 2차 승인 증빙   승인 2026-09-19 ════════════════════════════╡ ← 펼침 섹션(「차수 열기」와 같은 자리 · 한 번에 하나)
│ ▣ 승인메일_0919.pdf · 180KB · 09-19                        삭제        │ ← §7-10 파일 행
│ ┊ 파일을 끌어 놓거나 Ctrl+U · 이미지·PDF 10MB ┊                         │ ← 쓰기 권리가 있을 때만
```
...
### S22. 리저브 줄 증빙(D-60) — 리저브 대장 `/pnl/reserves` (04 S9 편집 표 + 표 아래 펼침 + §7-10 첨부 영역) — 리저브 쓰기 권한자

Phase 4에서 넘어온 D-60(리저브 줄 증빙 첨부). **새 컴포넌트 없음** — 04 S9 편집 표에 읽기 셀 열 하나, 펼침은 S21과 같은 규칙, §7-10 파일 행 · 첨부 영역(UA-621). 행 시트는 §7-3 (바) 보기 전용이라 거기에 업로드를 두지 않는다.

```
/pnl/reserves (04 S9, 편집 표 ≥1024)
│ 날짜   구분  금액        잔액        프로젝트  증빙 종류  세금계산서 번호  증빙     메모  클라이언트 │
│ 09-18  출금  3,300,000   12,700,000  아이오닉9 세금계산서  2026…-0012     증빙 2   …          │ ← 새 열 「증빙」(읽기 셀 + 3차)
╞═ 09-18 현대자동차 출금 3,300,000 증빙 ═══════════════════════════════════════╡ ← 표 아래 펼침 섹션(한 번에 하나)
│ ▣ 세금계산서_0918.pdf · 240KB · 09-18                                삭제   │
│ ┊ 파일을 끌어 놓거나 Ctrl+U · 이미지·PDF 10MB ┊                                 │
```
...
- **내용:** 편집 표의 variant `selectable`. 맨 왼쪽 선택 열(네이티브 체크박스 · `accent-color: var(--native-accent)`(Color 절과 같음) · 칸 폭 = 28 + `--cell-pad-x`×2(44, Spacing 절) ·
  ...
  **(rev 9 보강 — `/plan-design-review`)**
  ...
  - **처리 뒤 선택:** 서버가 막힌 행마다 「지금 고를 수 있음」을 다시 보낸다. 게이트로 막혀 이제 고를 수 없는 행은 선택을 풀고 이유 줄만 남기고, 여전히 고를 수 있는 행(동시성 · 재계산으로만 막힘)은 선택을 유지한다 — 비활성인데 체크된 행이 생기지 않고 1차 N은 고를 수 있는 선택만 센다(H-3).
  - **알림 · 포커스:** 결과 글자는 처음부터 DOM에 있는 `aria-live="polite"` 영역에 쓰고, 막힌 행 이유 줄 id를 그 행 체크박스의 `aria-describedby`에 더한다. 확인 모달이 닫히면 포커스 = 첫 막힌 행의 체크박스, 없으면 1차(DR-6).
...
| S21 | empty | ✅ resolved | explicit | 「Empty — 차수 승인 증빙(S21, 펼침 섹션)」 — 쓰기 권리면 첨부 영역만, 읽기 · 파일 0이면 `승인 증빙 없음 · 올리기는 담당 PM 박서연`. 승인 표시 취소 뒤 파일 0이면 3차 자체가 없다 |
| S21 | loading | ✅ resolved | explicit | 펼침은 04 S5 「차수 열기」와 같은 규칙(이 페이즈 새 진행 표시 없음). 업로드 = §7-10 행 아래 2px 진행 바, 삭제 = 누른 3차 `…`. 즉시 저장이라 상세의 `일괄 저장 Ctrl+S N`에 들지 않는다 |
| S21 | error | ✅ resolved | explicit | 「Error — 증빙 업로드」 — 중복은 늘 번호 없는 문구(같은 주인 종류 = 차수 승인 증빙 안에서만 막힘, O-6). 권리 밖 직접 호출은 서버가 거부한다(06-16) |
| S21 | populated | ✅ resolved | explicit | 와이어프레임대로 — 승인된 최신 차수 행 동작 칸 3차 `승인 증빙 {N}` → 차수 섹션 아래 펼침 제목 `상세 견적 2차 승인 증빙` + 부제 `승인 2026-09-19` · §7-10 파일 행(쓰기면 3차 `삭제`) + 첨부 영역. 성공 토스트 없음 |
| S21 | partial | ✅ resolved | explicit | 이전 차수 = 「차수 열기」 펼침 제목 아래 같은 블록(동작 칸 3차 하나). 승인 표시 취소 뒤 = 파일이 있을 때만 3차가 남고 펼치면 읽기. 읽기 권리 = 파일 행만(`크게 보기`). `quote.amount` 없는 사람 = 3차 · 블록 · 개수가 DOM에 없다 |
| S21 | overflow | 🧪 resolved | backstop | 파일 10개 차수에서 파일 행이 펼침 섹션 안에 페이지 없이 전부 서는지 CI=true DOM 감사 |
| S21 | zero-one-many | ✅ resolved | explicit | 3차 라벨 0 = `승인 증빙`(숫자 없음) · 1+ = `승인 증빙 {N}`, 접근 이름 `2차 승인 증빙 1`. 펼침은 한 번에 하나(「차수 열기」와 서로 바뀐다), 열리면 포커스 = 섹션 제목, 닫히면 그 3차 |
...
| S22 | zero-one-many | ✅ resolved | explicit | 0 = `증빙` · 1+ = `증빙 {N}`, 접근 이름 `09-18 현대자동차 증빙 2`. 펼침은 한 번에 하나(다른 줄 3차를 누르면 바뀐다). 편집 표 키에서 이 셀 `Enter` = 펼치기 |
...
| S1 일괄 결과 알림 | error | ✅ resolved | explicit | 막힌 행 이유 줄(「거부 — 일괄 지급 건별 결과」)의 id를 그 행 체크박스 `aria-describedby`에 더한다(증빙 칸 글자 id와 함께 — 누르기 전에 읽힌다) |
...
| Review | Runs | Status | Findings | 이 문서(rev 9) |
|---|---|---|---|---|
| CEO(`/plan-ceo-review`) | 앞 라운드 | clear | HOLD SCOPE · 「한 페이즈 유지」 · O-2 「막기」 | 잠김 — 다시 열지 않음. C-3(06-16 UI 표면을 디자인 검토로 미룸)은 이번 DR-1 · DR-2로 닫힘 |
| Eng(`/plan-eng-review`) | 앞 라운드 | 계획 쪽 | N-1(D-56 · D-60 설계 없음 — 「해결」 근거가 거짓 통과 grep) | S21 · S22로 해소(DR-1 · DR-2 · DR-11). `eng-review.md` 정정은 계획 쪽 |
| Design 본 검토 | Opus 7패스 | issues_found → 반영 | 막음 2 · 고침 5 · 참고 5 | 전부 반영(DR-5는 DR-4에 딸려 함께 닫힘) |
| Outside 교차 검토 | Opus(Codex 대체) | issues_found → 반영 | 고침 6(C-1 · H-1~H-5) · 참고 9(M-1~M-9) | M-8만 버림(아래), 나머지 반영 |
...
**버림 · 미룸(이유):**
- M-8(매출 섹션에서 안 쓰는 표 접기) — 버림. S16 「매출 섹션의 두 역할」: 접으면 PM이 `발행됨` 요청이 어느 발행 줄이 됐는지 볼 곳이 없어지고, Phase 4 S6 발행 줄 표 계약을 바꾸게 되며, SYSTEM에 「섹션 안 표 접기」 규칙이 없어 화면 하나만 예외가 된다. 할 수 있는 일은 이미 권한별 표 모양이 말한다.
- C-1 수정안 ④ 뒤 절반(「내 차례」 `확인 전 N건` 줄 → `evidence=unreviewed`) — 넣지 않음. §7-4는 한 줄에 다음 행동 하나이고, 확인은 목록 제자리(모달)에서 끝난다. 경영관리 지급 묶음 줄 그대로.
- 제자리 확인 뒤 증빙 칸 2행(`확인됨 · 14:02`) — 넣지 않음. 다른 `확인됨` 행과 모양을 같게 둔다.
- Component Inventory ConfirmDialog 사용처는 반영 지시 1의 「아홉」이 아니라 **여덟** — H-4가 모달 둘(카드 사용 삭제 · 요청자 본인 취소)을 빼고 증빙 확인 하나를 더한 결과.

**OUTSIDE COVERAGE:** Codex unavailable(사용 한도, 2026-09-29까지) — Opus 교차 검토로 대체. 한도가 풀리면 rev 9 Codex 재확인 필요.

**VERDICT:** 막음 2건 해소 · 고침 8건 반영 · 참고 14건 중 13건 반영 · 1건 버림(M-8, 이유 있음). `gsd-ui-checker` 변경 범위 재확인 대기.

**디자인 반영 r2(2026-09-25 — 반영 검사 `design-apply-cross-r1.md` · `design-apply-checker-r1.md`, 교차 검토는 Codex 대신 Opus):** rev 번호는 9 그대로 두고 문장만 고쳤다(계획들이 「rev 9」를 인용한다).
- B-1 → S1 「제자리 증빙 확인」 「서버」 · 「실패」 · 금액 표시 절 「낙관적 잠금」 · Copywriting 「거부 — 증빙 확인 모달(S1 · S3)」 · UI Considerations `S1 제자리 증빙 확인` error — PM 증빙 변경도 `version`을 올리고, 모달은 열 때 받은 `version`으로 확인하며, `새로 고침`은 모달을 새 증빙으로 다시 세운다.
- F-1(= 체커 W1) → Color Destructive 줄 · UI Considerations `S11` loading(본인 요청 취소 = 즉시, 남의 요청 취소 = 사유 모달).
- F-2 → S1 「문서 화면 왕복」 「되살리기」(보관한 id 전부를 합 요청 한 번으로 — 다른 쪽 선택도 산다. R-4 같이 닫힘).
- F-3 → S21 머리 「04 S5 개정」 ⒜ ⒝ ⒞.
- W2 → S14 「한쪽 연결」(3차 `카드 사용 등록` → `/cards?new=1&line={그 줄 id}`) · 카드 입구 줄 · UI Considerations `S14` partial.
- B-2(카드 전표 첨부 주인 플랜) → 계획 쪽 06-25(오케스트레이터 결정). S7 · S8 · S9 · S13 문장은 그대로다.

**사용자 확정(2026-09-25) — 열린 선택 표(확정):**
