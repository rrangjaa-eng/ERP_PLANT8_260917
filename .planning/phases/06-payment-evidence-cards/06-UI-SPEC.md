...
견적 줄 행(S18 미매칭) `6 해외 연사 항공·숙박 지출결의 올리기` / 발행 요청 행 S16 `희망 09-30 발행 줄로` · S17 `26001 희망 09-30 발행 줄 입력` · `26001 희망 09-30 요청 보기`(프로젝트 번호 + 희망일) |
...
| 표시 — 점검 행 | 미결 지출결의: 번호 · 항목 · 금액 · 상태 글자(`결재 중` / `반려` / `증빙 없음` / `선결제 · 증빙 없음` — 증빙 `면제` 문서는 선결제·지급 뒤여도 행이 없다, D-611) · 3차 `지출결의 열기`. 미매칭 줄: 번호 · 항목 · 실행가 · 담당 PM에게 3차(그 줄의 문 — `지출결의 올리기` 또는 `구매 요청`), 그 밖의 사람에게 `담당 PM 박서연`. 매출 미입력: `발행 줄 없음` · 담당 PM에게 3차 `발행 요청 추가` / 매출 기록 권한자에게 3차 `발행 줄 추가` |
...
**Approval:** approved — gsd-ui-checker round 3/4(rev 4), 2026-09-24 (D1~D6) · rev 5 D1~D7 PASS. rev 5는 Codex 최종 리뷰 blocker 4건 반영본이다. rev 6은 Codex 전체 리뷰 R5-B1·R5-B2 + 작은 지적 2건 + checker 메모 반영본이다(checker 재검 전)
