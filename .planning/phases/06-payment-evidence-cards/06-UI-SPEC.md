...
| ConfirmDialog | `ui/confirm-dialog/ConfirmDialog` | 사용처 일곱(S2 일괄 지급 완료 · S5 지급 취소 · S4 증빙 면제 · S11 구매 요청 취소 · S9 카드 사용 삭제 · S16 발행 요청 취소 · S9·S12 입력 버리기). 전부 기존 슬롯 안(확인 근거 한 칸 = 날짜 또는 사유, §7-8 882행) — 새 슬롯 없음(UA-602) |
...
1. **「내 차례」 태그·행동 링크 + `--accent` 상태 글자** — ⓐ 3차 버튼: `증빙 확인`(목록 행의 문서 화면 링크) · `번호 보기`/`가리기` · `지급 예정일 바꾸기` · `예정일 저장` · `증빙 면제` · `바꾸기`(증빙 금액·카드·연결·거래처) · `구매 완료` · `요청 취소` · `구매 요청 3`/`발행 요청 2`(하위 목록 링크) · `발행 요청 추가` · `발행 줄로` · `발행 줄 입력` · `점검 보기` · 점검 행의 `지출결의 올리기`/`카드 사용 등록`/`구매 요청`/`증빙 올리기` · `필터 지우기` · 페이지 번호·`이전`·`다음` · `새로 고침` ·
...
**체크박스(선택 열)의 `accent-color`는 `--native-accent`이고 다섯 곳 규칙과 별개다**(§7-13 946행과 같은 해석). **진행 바는 시스템 판 그대로
쓰고 새로 더하지 않는다** — §7-1(688행, 300ms 넘는 서버 액션: 상단 바 아래 2px) · §7-7 LOADING(853행) · §7-10 업로드 행(904행). 그 `--accent`는
세 절이 정한 시스템 요소라 이 다섯 곳 목록이 새로 쓰는 자리가 아니다. Phase 4의 CEO-D17(진행 바 없음)은 그 페이즈 범위의 결정이고 §1-3과의 충돌은 04-08이
...
| Primary CTA — 지출결의 문서 화면(S4·S5, 지급 권한자) | 상태에 따라 **하나만**: 증빙 `확인 전` → `증빙 확인` + `Ctrl+Enter` / 증빙 `확인됨`·`면제`·`선결제`, 또는 증빙 필수 off → `지급 완료` + `Ctrl+Enter` / 지급 완료 뒤 → 1차 없음(2차 `지급 취소`) —
단 지급 뒤 들어온 증빙이 `확인 전`이면(선결제) 1차 `증빙 확인` + `Ctrl+Enter`(확인만 기록, 지급 칸·지급 행동은 다시 서지 않는다) + 2차 `지급 취소` / 증빙 `없음` → 1차 `지급 완료` **비활성** + 이유 `증빙 없음 · 담당 PM 박서연`(`block`), 3차 `증빙 면제`는 증빙 섹션에 따로 |
...
| 3차 버튼 | `증빙 확인`(목록 행 → 문서 화면 증빙 섹션) · `증빙 면제` · `바꾸기`(증빙 금액 · 카드 · 연결 · 거래처 · 프로젝트 · 견적 줄) · `지급 예정일 바꾸기` · `예정일 저장`(제자리 편집, `Enter`) · `견적 외 비용으로` / `팀 비용으로`(S10 막힘의 다음 한 수) · `번호 보기` / `가리기` · `구매 요청 3` / `발행 요청 2`(하위 목록 링크, 건수 = 처리 대기) · `구매 완료` · `요청 취소` · `발행 요청 추가` · `발행 줄로` · `발행 줄 입력` · `점검 보기` · `지출결의 열기` · `지출결의 올리기` · `구매 요청` · `카드 사용 등록` · `발행 줄 추가` · `지급 대상 보기` · `지급 완료 보기` · `구매 요청 보기` · `발행 요청 보기` · `전체 보기` · `필터 지우기` · `새로 고침` · 페이지 `이전`/`다음`/번호 · 구매 요청 품목의 링크(external-link **아이콘만**, 글자 없음 · `aria-label="링크 열기"` · `title` = URL). 같은 낱말 `바꾸기`가
한 화면에 여럿이면 각 버튼에 `aria-label="{칸 라벨} 바꾸기"`(`가맹점 바꾸기` · `프로젝트 바꾸기` · `견적 줄 바꾸기` · `증빙 금액 바꾸기`) |
...
| Empty — 연결 고르기 목록(S10) | 프로젝트에 견적 줄 0: `이 프로젝트에 견적 줄이 없습니다 · 견적 외 비용으로`(3차 — 연결 라디오를 「견적 외 비용」으로 바꾸고 목록을 닫는다) / 검색 0건: `조건에 맞는 줄이 없습니다 · 검색 지우기` / **줄은 있으나 고를 수 있는 줄 0**(전부 반대쪽, 또는 구매 요청 폼에서 온라인구매 줄 0):
...
| 거부 — 문서 화면 응답 없음(S4·S5) | 행동 줄 이유 자리 `결과를 받지 못함 · 새로 고침`(`block`) — 증빙 확인 · 지급 완료 · 지급 취소 · 지급 예정일 저장 공통. 칸의 입력값은 남는다 |
...
| 표시 — 문서 화면 「지급」 섹션(지급 완료 뒤, 모두에게) | `지급 방식` `계좌이체` / `지급일` `2026-09-19` · 2행 `이과장` / `이체액`(법인카드면 `카드 결제액`, 현금이면 `현금 지급액`) `13,636,700` · 2행 `지급 총액 13,640,000 · 차이 -3,300 · 공급가 역산 12,397,000`(역산은 부가세 규칙일 때만 — 금액 표시 절) / (차이가 있으면) `차이 사유` `이체 수수료` |
...
| 표시 — 증빙 금액(S4) | 값 `12,400,000` + 2행 `박서연 09-17`(입력한 사람·날) + 서버 계산 한 줄 `부가세 1,240,000 · 지급 총액 13,640,000 · 세금계산서 규칙`(§6-3 423행 꼴, 숫자만 700. 세율 `%` 글자를 쓰지 않는다 — 세율은 기준일의 설정 값이고 금액이 서버 보간값이다) |
...
| 표시 — 지급 방식 · 계좌(S1 열 · S5 칸) | 계좌이체: `신한 ****-**-1234` + 2행 예금주 `(주)스테이지원`(계좌가 곧 방식 표시). 권한자만 3차 `번호 보기` → 평문 + `가리기`(UA-613). 계좌이체인데 계좌가 없으면 `계좌 없음`(`--muted`). 법인카드: `법인카드` · 현금: `현금`(계좌 글자·`번호 보기` 없음, UA-618) |
...
| 표시 — 카드 폼 연결 「팀 비용」 | 값 `기획1팀` + `Form.Hint` `사용일 소속`. 팀은 **사용한 사람의 사용일 소속 팀**(`teamAtDate()`, 06-CONTEXT:27)이고 카드 팀이 아니다 — 팀 카드도 같다. 사용한 사람 = 본인 등록이면 등록자, 대리 등록이면 `사용한 사람` 칸(S9)의 사람 → 힌트 `김OO 사용일 소속` |
...
| 제자리 편집 — 지급 예정일 바꾸기(S5, 모달 없음) | 3차 `지급 예정일 바꾸기` → 값 자리가 날짜 칸(280, 기본 지금 값)으로 바뀌고 옆 3차 `예정일 저장 Enter`. 원천징수·회사 대납 규칙일 때만 `Form.Hint` 결과 미리보기 `지급 총액 13,640,000 → 13,652,000`(D-101 — 지급 전에는 지급 예정일이 세율 기준일. 서버가 새 날짜로 계산해 보낸다, 같으면 힌트 없음). `Esc` = 서버 값으로 되돌림. 되돌릴 수 있는 날짜 편집이라 확인 모달을 쓰지 않는다(CLAUDE.md §7 · S4 증빙 금액 「바꾸기」와 같은 결) |
...
- **열(≥1280, 10열):** 선택 · 번호 · 프로젝트 · 항목(팀 이름 지출이면 `기획1팀 · 팀 관리비`) · 거래처 · **지급 방식**(Copywriting 「표시 — 지급 방식 · 계좌」: 계좌이체면
  1행 `{은행} ****-**-{뒤4}`, 2행 예금주 + 권한자 3차 `번호 보기`(UA-613) / `법인카드` / `현금`, UA-618) · 지급 예정일(`MM-DD`) · 증빙(상태 글자 + 같은 칸 3차/담당, Color 매핑표) ·
  지급 총액(서버 계산 payable, 읽기 전용) · 이체액(편집, 기본값 = 지급 총액, D-604 — 방식과 관계없이 실제로 나간 돈) · 차이 사유(이체액 ≠ 지급 총액일 때만 편집 가능, 아니면 `—` 읽기 전용).
- **열 우선순위(§7-3 · §6-1 361행 「숨기는 열은 P3에서만」):** P1 = 항목 · 지급 총액 · 증빙 / P2 = 예정일 · 지급 방식(1행만) / P3 = 번호 · 거래처 · 이체액 · 차이 사유.
...
- **좁은 PC:** 1024~1279 = P3 중 번호·거래처 숨김(8열 — 예금주는 지급 방식 2행에 남는다). 700~1023 = **보기 전용**(선택 열·이체액 편집·1차 없음, DR-36),
  열 = P1 + P2(항목 · 예정일 · 증빙 · 지급 총액 · 지급 방식). <700 = 폰.
...
- 합계 줄은 필터 전체 기준(§6-1). 오른쪽 `고른 N건 · 이체액 …`의 **이체액 합은 서버가 더한다**(06-CONTEXT:107 금액 산술 경계 — 클라이언트 합·린트 예외 없음, O-18 닫힘):
  ...
  둔다(금액 표시 절 「서버 계산 한 줄」과 같은 규칙). 건수 `N건`은 셈이 아니라 개수라 화면이 센다. S2 모달 부제의 이체액 합계도 같은 서버 값이다.
...
- 지급 권한자·대표: 열 = 번호 · 프로젝트 · 항목 · 거래처 · 지급일 · 이체액(2행 `차이 -3,300`, 있을 때만) · 차이 사유 · 증빙(상태 글자 — 지급 뒤 들어온 선결제 증빙이 `확인 전`이면 같은 칸 3차 `증빙 확인` → 문서 화면, S4) · 처리한 사람. 그룹 = 지급일의 주(`이번 주` → `지난주` →
  `09-01 ~ 09-07` …). 필터 `[지급 완료 ▾] [팀 ▾] [2026-09 ▾]`. 합계 줄 `합계 (지급 완료 · 2026-09 · 41건)` · `이체액 412,300,000`. **1차 없음.** 행 → 문서 화면(지급 취소는 거기서, S5).
  폰 P1 = 항목 · 이체액 · 지급일, P2 = 번호 · 거래처 · 증빙, P3 = 차이 사유 · 처리한 사람.
...
- **지급 뒤 들어온 증빙(선결제, D-602·D-603):** PM이 지급 뒤 증빙을 올리면 확인 줄이 `확인 전`이 되고, 지급 권한자에게 행동 줄 1차 `증빙 확인`이 선다(2차 `지급 취소`는
  그대로). 금액 `바꾸기`도 `확인 전`이라 선다. 누르면 확인만 기록된다 — 지급 상태·지급 칸·이체액은 그대로이고 지급 행동이 다시 서지 않는다. 선결제 표시는 `확인됨`으로
...
|---|---|---|---|
| 지급 예정일 | 값 + 3차 `지급 예정일 바꾸기` → 제자리 날짜 칸(모달 없음, Copywriting 「제자리 편집 — 지급 예정일 바꾸기」, UA-617) | 값(읽기) | 값 + 2행 `지급은 경영관리`(지급 전) |
...
| 이체액(라벨은 방식 이름 — 계좌이체 `이체액` · 법인카드 `카드 결제액` · 현금 `현금 지급액`) | 1차가 `지급 완료`일 때만 입력 칸(280, 기본값 = 지급 총액) + `Form.Hint` 차이(`지급 총액 13,640,000 · 차이 -3,300`, 같으면 `지급 총액과 같음` 없이 줄 없음). 1차가 `증빙 확인`이면 값 `13,640,000`(읽기) | `13,636,700` · 2행 `지급 총액 13,640,000 · 차이 -3,300 · 공급가 역산 12,397,000`(역산은 부가세 규칙만) | 지급 뒤에만 지급 권한자와 같은 읽기 줄 |
...
- **연결(EXP-07):** 네이티브 라디오 셋(`<fieldset>`, 라벨 열 = `연결`). 처음엔 아무것도 고르지 않은 상태 — 고르기 전에는 1차가 막힌다. 견적 줄 행에서 들어오면(S14) `견적 줄`이
  ...
  - `팀 비용`: 값 텍스트 `기획1팀` + `Form.Hint`(Copywriting 「표시 — 카드 폼 연결 「팀 비용」」). 팀을 고르는 칸 없음(자동, EXP-07 — 사용한 사람의 사용일 소속).
    **대리 등록일 때만** 그 위에 `사용한 사람` `Select`(200)가 선다 — 개인 카드면 기본값 = 카드 소지자(대개 그대로), 팀 카드면 그 카드 팀의 사람 중에서 고르고 기본값 없음
    (비면 §7-15 빈 칸 막힘에 든다). 본인 등록이면 칸이 없고 등록자가 사용한 사람이다.
...
- 열: 번호(`26001-C0001`, A-612) · 요청일 · 품목(+ 같은 칸 링크 아이콘 3차 — external-link 아이콘만, 글자 없음, `aria-label="링크 열기"` · `title` = URL, 새 탭 `rel="noopener noreferrer"`, http·https만) · 연결 · 요청자 ·
...
- 열: 희망 발행일 · 프로젝트(번호 · 이름) · 요청자 · 금액(2행 합계) · 메모 · 상태 + 행동 3차 `발행 줄 입력`(≥1024 · 매출 기록 권한자만 → `/projects/{id}?issueRequest={rid}`). 합계 줄 `합계 (요청 · 2건)` · `금액 44,000,000`.
...
| S17 발행 요청 목록 | 전부 · 행 3차 `발행 줄 입력` | 없음(자기 요청은 상세 표에서 본다) | 없음 | 없음 | 읽기 · 프로젝트 목록의 3차 `발행 요청` 링크로 들어온다(S17 진입) |
...
| 값 | 계산 | 어디서 보이나 |
|---|---|---|
| 지급 총액 | `applyTaxRule()`의 지급액(증빙 금액 + 증빙 종류 세금 규칙). 사람이 적지 않는다. **증빙 금액이 없으면**(증빙 면제 · 선결제 · 증빙 필수 off) 승인액을 같은 `applyTaxRule()`에 넣는다(증빙 > 승인액, D-605 순서). 원천징수·회사 대납 규칙의 기준일은 지급 전 지급 예정일 · 처리 때 지급일(D-101, UA-617) | S1 열 · S4 서버 계산 한 줄 · S5 이체액 힌트 |
...
| 공급가 역산 | **표시 전용(사후 참고) — 비용은 증빙 금액/승인액 그대로다**(D-605, 차액을 비용에 넣으려면 조정 줄 D-83). 규칙별: `vat_surcharge`만 이체액에서 `grossFromTotal()` 역산해 보인다. `none`·`company_borne`은 지급액 = 공급가라(`tax.ts:75·127`) 역산 값이 이체액과 같아 줄을 두지 않는다. `withholding`은 `domain/money`에 역산 함수가 없고 이 페이즈는 계산을 새로 쓰지 않으므로(06-CONTEXT:31) 줄을 두지 않는다 | S5 지급 뒤 2행(부가세 규칙만) |
| 카드 공급가 · 부가세 | 규칙별: `vat_surcharge` = 결제 합계에서 `grossFromTotal()` 역산(D-607) · `none` = 공급가 = 결제 합계(`규칙 없음`). 카드 결제에 원천징수·회사 대납은 없다 — 그 규칙의 증빙 종류는 카드 폼 옵션에서 서버가 빼서 보낸다(계획이 코드표로 확인). 역산 반올림 잔차(`recomputeDeltaKrw`, 매출 입금 역산 선례 RESEARCH 312행)가 0이 아니면 서버 계산 한 줄 끝 `· 반올림 차이 N`. 입력 칸 없음 | S8 2행 · S9 서버 계산 한 줄 · S15 2행 |
...
> **loading은 시스템 판 그대로이고 이 페이즈가 새 진행 표시를 더하지 않는다**(Color 절): §7-7 LOADING(상단 바 아래 2px 진행 바 + 머리글 + `--surface` 행 3개,
> 반짝임 없음, 300ms 지연 표시) · 누른 버튼의 `…`와 300ms 넘으면 상단 2px 진행 바(§7-1) · 첨부 업로드 행 아래 2px 진행 바(§7-10).
...
| partial | S7 여러 파일 업로드 | ✅ covered | 성공 행 + 실패 행 각각(§7-7 첨부 PARTIAL) · 「Error — 증빙 업로드」 4종 · 중복은 읽을 수 있는 문서일 때만 번호, 아니면 번호 없는 문구 |
...
| empty | S18 완료 전 점검 | ✅ covered | 세 그룹이 다 비었을 때만 그룹 자리 한 줄 `미결 없음`. 막힘 0이어도 강행 허용 그룹 항목과 매출 줄(차이, D-613)은 남는다. 정산 결재 올리기 버튼 살아 있음 |
