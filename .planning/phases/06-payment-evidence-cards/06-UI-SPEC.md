...
**지급 예정일 칸이 dirty인 동안은 위 상태와 관계없이 1차 = `예정일 저장` + `Ctrl+Enter`**(SP-3 — 저장하거나 `Esc`로 되돌리면 상태의 1차로 돌아온다. 증빙 없음으로 `지급 완료`가 막힌 문서도 예정일은 이 1차로 저장된다) |
...
| 3차 버튼 | `증빙 확인`(목록 행 → 문서 화면 증빙 섹션) · `증빙 면제` · `바꾸기`(증빙 금액 · 카드 · 연결 · 거래처 · 프로젝트 · 견적 줄) · `고르기`(빈 가맹점) · `지급 예정일 바꾸기`(저장은 화면 1차 `예정일 저장`, SP-3) · `견적 외 비용으로` / `팀 비용으로`(S10 막힘·EMPTY의 다음 한 수) · `검색 지우기`(S10) · `번호 보기` / `가리기` · `구매 요청 3` / `발행 요청 2`(하위 목록 링크, 건수 = 처리 대기) · `구매 완료` · `요청 취소` · `발행 요청 추가` · `발행 줄로` · `발행 줄 입력` · `점검 보기` · `지출결의 열기` · `지출결의 올리기` · `구매 요청` · `카드 사용 등록` · `증빙 올리기` · `발행 줄 추가` · `지급 대상 보기` · `지급 완료 보기` · `구매 요청 보기` · `발행 요청 보기` · `전체 보기` · `필터 지우기` · `새로 고침` · 페이지 `이전`/`다음`/번호 · 구매 요청 품목의 링크(external-link **아이콘만**, 글자 없음 · `aria-label="{품목} 링크 열기"` · `title` = URL) · 막힘 줄의 다음 한 수(`카드 고르기` · `연결 고르기` · `증빙 종류 고르기` · `품목 적기` · `사유 적기` · `지출결의로`). 같은 낱말 `바꾸기`가
...
견적 줄 행(S18 미매칭) `6 해외 연사 항공·숙박 지출결의 올리기` / 발행 요청 행 S16 `희망 09-30 발행 줄로` · S17 `26001 희망 09-30 발행 줄 입력`(프로젝트 번호 + 희망일) |
...
| 막힘 — 카드 사용 폼(`Form.Actions`) | 빈 칸: `카드 · 결제 합계 2칸 비어 있음 · 카드 고르기`(§7-15 원문 꼴) · 연결 미선택: `연결 없음 · 연결 고르기` · 완료 프로젝트(경영관리 아님): `완료 · 견적 줄 잠김` · 서버 이중 연결 거부(D-609, 경합): `지출결의 26001-0004 연결됨 · 다른 줄 고르기` · 저장된 증빙 종류가 카드 옵션에 없음(수정 모드): `증빙 종류 사업소득 카드에 없음 · 증빙 종류 고르기` · 팀 비용인데 사용일 소속 없음: `김OO 09-18 소속 없음 · 소속 발령은 관리자`(버튼 없음) |
...
| 거부 — 이체액 합계 응답 없음(S1·S2) | S1 합계 줄 오른쪽 `고른 5건 · 합계를 받지 못함 · 다시 시도` / S2 모달 막힘 이유 자리 `합계를 받지 못함 · 다시 시도`(`block`, 1차 `aria-disabled`). `다시 시도` = 2차(§7-7 로드 오류와 같은 버튼) — 같은 스냅숏으로 합을 다시 부른다 |
...
| 표시 — 카드 폼 가맹점 기본 증빙이 카드에 없음 | 가맹점 줄 아래 `Form.Hint` `기본 증빙 사업소득 · 카드에 없음`(증빙 종류는 `카드 전표`로 채워진다 — S9 「증빙 종류 옵션과 거래처 기본값」) |
...
| SUCCESS — 제자리 결과(토스트 없음) | 증빙 확인 → 증빙 확인 줄 `확인됨 · 이과장 09-18 14:02` + 다음 1차는 지급 상태로 갈린다: **지급 전 문서**면 1차가 `지급 완료`로 바뀜 / **지급 뒤 문서**(지급 뒤 들어온 선결제 증빙, S4)면 1차가 사라지고 2차 `지급 취소`만 남는다 — 지급 칸·지급 행동은 다시 서지 않는다 · 증빙 면제 → `면제 · …` · 예정일 저장 → 값이 바뀌고 날짜 칸이 읽기 값으로 닫히며 1차가 상태의 것으로 돌아옴(목록이면 그룹 이동) · 발행 요청·「발행 줄로」 → 상세 합계 행 `저장됨 N줄 14:02`(Phase 4) · 한 건 지급 완료 → 1차 자리 `지급 완료 → 2026-09-19 · 14:02`(`--success`) + 토스트 |
...
- **1차 `지급 완료 N`:** N = 고른 건수(모든 쪽). 누르면 → 고른 행에 오류 칸이 있으면 첫 오류 칸으로 포커스(모달 없음) → 없으면 그 순간의 **스냅숏**(고른 행 id + 행마다 이체액·차이 사유 편집값)을 들고 S2 확인 모달.
  모달이 열려 있는 동안 표는 편집되지 않으므로(§7-17) 스냅숏이 바뀌지 않는다.
...
- 합계 줄은 필터 전체 기준(§6-1). 오른쪽 `고른 N건 · 이체액 …`의 **이체액 합은 서버가 더한다**(06-CONTEXT:107 금액 산술 경계 — 클라이언트 합·린트 예외 없음, O-18 닫힘):
  ...
  **스냅숏 묶기:** 요청마다 스냅숏 번호(선택·편집이 바뀔 때마다 1 늘어나는 수)를 싣고, 응답의 번호가 지금 스냅숏의 번호가 아니면 버린다(늦게 온 옛 합이 새 선택의 합으로 보이지 않는다).
  S2 모달 부제의 이체액 합계는 **모달을 연 스냅숏의 합만** 쓴다 — 그 합이 이미 와 있으면 바로, 아직이면 부제 합계 자리 `…` + 모달 1차 `…` · `aria-disabled`(`Ctrl+Enter` 무반응)로
  기다렸다가 그 번호의 응답이 오면 채우고 1차를 켠다. `--faint` 이전 값은 모달에 옮기지 않는다. 합 요청이 실패하거나 응답이 없으면 Copywriting 「거부 — 이체액 합계 응답 없음」
  (S1 합계 줄 · S2 막힘 이유 자리) — 1차는 그 스냅숏의 합이 올 때까지 켜지지 않는다.
...
- 부제 합계와 1차가 켜지는 때: S1 「합계 줄」의 스냅숏 묶기 그대로(모달을 연 스냅숏의 서버 합이 올 때까지 1차 `aria-disabled`, 실패하면 「거부 — 이체액 합계 응답 없음」).
  처리 요청도 **같은 스냅숏**(행 id · 이체액 · 차이 사유)을 보낸다 — 부제에 보인 합과 처리한 값이 어긋나지 않는다.
...
- **지급 뒤 들어온 증빙(선결제, D-602·D-603):** PM이 지급 뒤 증빙을 올리면 확인 줄이 `확인 전`이 되고, 지급 권한자에게 행동 줄 1차 `증빙 확인`이 선다(2차 `지급 취소`는
  그대로). 금액 `바꾸기`도 `확인 전`이라 선다. 누르면 확인만 기록된다 — 지급 상태·지급 칸·이체액은 그대로이고 지급 행동이 다시 서지 않는다(확인 뒤 행동 줄 = 1차 없음 + 2차 `지급 취소`, Copywriting 「SUCCESS — 제자리 결과」의 지급 뒤 갈래). 선결제 표시는 `확인됨`으로
...
- 1차 규칙은 Copywriting 「Primary CTA — 지출결의 문서 화면」(지급 예정일 칸이 dirty인 동안은 `예정일 저장` — 키·되돌리기는 「제자리 편집 — 지급 예정일 바꾸기」). 한 건 지급 완료는 **확인 모달 없이** 행동 줄 1차로 끝난다(지급일·이체액이 이미 화면 칸이고, 되돌리기 =
...
- **증빙 종류 옵션과 거래처 기본값(금액 표시 절 「카드 공급가 · 부가세」):** 옵션은 서버가 카드에 맞는 규칙(`vat_surcharge` · `none`)의 종류만 보낸다. 거래처 기본 증빙 종류가
  ...
  사람이 고를 수 있는 값은 옵션 안의 것뿐이라 카드에 없는 규칙으로 저장될 길이 없다.
...
| 값 | 계산 | 어디서 보이나 |
|---|---|---|
| 지급 총액 | `applyTaxRule()`의 지급액(증빙 금액 + 증빙 종류 세금 규칙). 사람이 적지 않는다. **증빙 금액이 없으면**(증빙 면제 · 선결제 · 증빙 필수 off) 승인액을 같은 `applyTaxRule()`에 넣는다(증빙 > 승인액, D-605 순서). 두 기준일(`tax.ts:30-35`)은 호출자 `domain/payments`가 아래 「기준일 고르기」대로 고른다(RESEARCH Pattern 2) | S1 열 · S4 서버 계산 한 줄 · S5 이체액 힌트 |
...
- **기준일 고르기(지급 총액 — 모든 갈래에 날짜가 하나씩 정해진다):**
  ...
  - 지급 뒤 보이는 지급 총액은 처리 때 위 날짜로 계산해 문서에 남긴 값이다. 지급 뒤 들어온 증빙(선결제)은 확인만 기록하고 지급 총액·차이를 다시 계산하지 않는다(S4 「지급 뒤 들어온 증빙」,
    기본값 — 계획이 `domain/payments` 저장 칸으로 확인).
...
> UI-consideration probe의 해소 결과(rev 1, rev 3에서 네 줄 더함). 이 절은 **상태 커버리지**만 다룬다. 문구는 `## Copywriting Contract`에 있고 여기서는 그 행 이름을 가리키기만 한다.
...
Applicable state considerations resolved: 44 covered, 5 backstop, 2 unresolved
...
| error | S1·S2 고른 건 이체액 합계 | ✅ covered | 요청마다 스냅숏 번호 — 옛 번호 응답은 버린다. S2 1차는 모달을 연 스냅숏의 합이 올 때까지 `…` · `aria-disabled`. 실패 = 「거부 — 이체액 합계 응답 없음」(`다시 시도`) |
...
| partial | S5 지급 예정일 편집 중 | ✅ covered | dirty인 동안 1차 = `예정일 저장 Ctrl+Enter`, 칸의 `Enter`는 제출 안 함, `Esc` = 되돌리고 상태의 1차로(SP-3 ②). 다른 칸 입력값은 남는다 |
...
| error | S9 증빙 종류가 카드에 없는 규칙 | ✅ covered | 거래처 기본값이면 `카드 전표`로 채우고 가맹점 `Form.Hint`로 알린다 · 저장된 건이면 `Select` `—` + 1차 막힘(「막힘 — 카드 사용 폼」) · 서버도 거부 |
| zero-one-many | S9 사용한 사람 후보(대리 등록 · 팀 비용) | ✅ covered | 개인 카드 = 소지자 텍스트 · 팀 카드 = 사용일 소속 후보(옮긴 사람·퇴사자 포함), 1명이면 텍스트 · 여럿이면 `Select` 기본값 없음 · 사용일 소속 없음 = 1차 막힘 |
