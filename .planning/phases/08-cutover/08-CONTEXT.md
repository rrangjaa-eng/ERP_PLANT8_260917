...
**2026-09-24 페이즈 분리:** OPS-03과 D8-08(복원 리허설 워크플로·복원 절차 문서·상태 화면 「복원 리허설」 항목), D8-07의 표시 부분(사람 목록 「첫 로그인 전」·「임시 비밀번호 사용 중」)은 Phase 04.4 「복원 리허설·로그인 상태」로 옮겼고 결정은 `.planning/phases/04.4-restore-rehearsal-and-login-status/04.4-CONTEXT.md`에 복사했다. 이 페이즈에는 demo 행 삭제·전환 표시(D8-05)·번호 카운터(D8-09)·연습용 마스터 보관(D8-06)·전환일과 경계 업무(D8-01~D8-04)·D8-07의 운영 규칙(안 바꾼 사람이 있어도 전환)·전환일 체크리스트와 MIG-04·MIG-05가 남는다(`.planning/ROADMAP.md` Phase 8 기준 1~4, 기준 5는 체크리스트의 결과 확인만). 아래 결정은 논의 기록으로 그대로 둔다.
...
### 계정·로그인 확인 (표시 → Phase 04.4로 옮김, 운영 규칙은 이 페이즈)
