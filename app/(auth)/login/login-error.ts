...
// APIError("FORBIDDEN", { message: lockedMessage(windowMinutes) })로 403을 던지고, 그 문구는
...
// 이제 고정 접두어·접미어로 대조한다(분 숫자는 설정값). 판정 함수는 import 없는 잎 모듈에 있어
// lib/env를 클라이언트 번들로 끌고 오지 않는다(domain/auth/locked-message.ts).
import { isLockedMessage } from "@/domain/auth/locked-message";
...
export function loginErrorMessage(
  ...
  if (error?.status === 403 && typeof error.message === "string" && isLockedMessage(error.message)) {
