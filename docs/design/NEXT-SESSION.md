# 다음 세션 — §4(통일) 계속: B(Pretendard) · C(§6 남은 실물)
...
- PR #8(시트·모달) 머지됨. 이번 작업은 브랜치 `design/print-redesign-260918`, 드래프트 PR → `main`(GitHub 기본 브랜치 이름은 `main`). 머지 전이면 그 브랜치를 체크아웃
...
2. 아래 **B · C** 중 무엇을 할지(A는 끝) **사용자에게 묻는다**(한 번에 한 질문). 사용자가 고른 것만 한다
