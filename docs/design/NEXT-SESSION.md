# 다음 세션 — §4(통일): **C 끝. 인쇄 확인증 경품 반영 여부 → B(Pretendard)**
...
2. C는 전부 끝. **먼저 한 건 묻는다**: 인쇄 「기타소득 지급 확인증」(§6-6, `print-cert.html`)이 아직 지급액 · 원천징수 8.8% · 계좌 골격인데, 경품이 대부분이므로 경품명 · 시가 · 원천징수 22%로 바꿀지(내부·세무 보관용이라 세금 줄은 남는다). 그다음 B를 할지 묻는다
...
### 다음 세션이 할 일
1. 인쇄 확인증 경품 반영 여부를 묻는다(위 1-2). 바꾸면 `print-cert.html` 다시 그려 보드 → 확정 → §6-6 · DECISIONS
2. B(Pretendard) 할지 묻는다
3. C에서 배운 것: 사용자는 실물을 보고서야 전제(현금 vs 경품)를 말한다 — 첫 보드에 **업무 전제를 한 줄로 적어 두고** 「이 전제가 맞나」부터 묻는다. 「다 빼라」는 말은 글자 그대로 뺀다(안내 문구까지)
