# 다음 세션 — §4(통일) 계속: **C-2 손익 원장 · C-3 외부 수령자 보드 확인** · 그다음 B
...
- 실물이 있는 것: 원장 화면(`system/preview.html`) · 시트·모달(`system/sheet-modal.html`) · 인쇄 템플릿 2종(`system/print-expense.html` · `print-cert.html`, 2026-09-18 재디자인 확정) · **폼 화면(`system/form-expense.html`, 2026-09-18 확정 — C-1 끝)**
...
2. **C-2 손익 원장 보드부터**(C-1 폼은 끝). C-3까지 끝나면 B를 할지 묻는다
...
## C. §6 템플릿 중 남은 실물 — **C-1 폼 끝(2026-09-18), C-2·C-3 초안 있음, 확인 전**

C-1 폼은 보드(https://claude.ai/artifact/Ts4ScSj1hD2AiiWD17Evkc)로 확정했다 — `DECISIONS.md` 「폼 화면 실물 확정 7건」, `SYSTEM.md` §6-3. 남은 초안 둘(`system/dashboard-pnl.html` · `external-cert.html`)은 **아직 사용자가 본 적 없다 — 아무것도 확정되지 않았다.** 스크린샷은 찍어 두었고 어긋난 곳은 고쳤다(2026-09-18).

### 다음 세션이 할 일 (이 순서로, 한 화면씩)
1. `node docs/design/system/shoot.mjs pnl-ceo pnl-lead pnl-empty pnl-m pnl-m-view cert-empty cert-sign cert-error cert-done cert-expired` → PNG 10장(이미 `shots/`에 있음)
2. **C-2 손익 원장 보드** → 사용자에게 「이대로 확정?」 한 질문 → 답에 따라 아래 물을 것 (a)~(b)를 한 건씩 → DECISIONS·SYSTEM 갱신
3. **C-3 외부 수령자 보드** → 같은 절차
4. 둘 다 끝나면 커밋·PR, 그다음 B(Pretendard)
5. C-1에서 배운 것: 「확정인데 더 예쁘게」가 나오면 시스템 안에서 구도만 다듬은 전·후를 같은 보드에 올려 고르게 한다. 두 갈래 결정은 실물 2안을 만들어 나란히 보인다(요소 캡처는 `shoot.mjs` 9번째 필드)
