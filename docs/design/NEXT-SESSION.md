# 다음 세션 — §4(통일) 계속: **C-3 외부 수령자 보드 확인** · 그다음 B
...
- 실물이 있는 것: 원장 화면(`system/preview.html`) · 시트·모달(`system/sheet-modal.html`) · 인쇄 템플릿 2종(`system/print-expense.html` · `print-cert.html`, 2026-09-18 재디자인 확정) · **폼 화면(`system/form-expense.html`, C-1 끝)** · **손익 대시보드 전사·팀(`system/dashboard-ceo.html` · `dashboard-team.html`, C-2 끝 — 「대시보드도 원장」을 사용자가 뒤집음, `DECISIONS.md` 손익 화면 = 대시보드)**
...
2. **C-3 외부 수령자 보드부터**(C-1 폼 · C-2 손익은 끝). 끝나면 B를 할지 묻는다
...
## C. §6 템플릿 중 남은 실물 — **C-1 폼 · C-2 손익 끝(2026-09-18), C-3 초안 있음, 확인 전**

C-1 폼은 보드(https://claude.ai/artifact/Ts4ScSj1hD2AiiWD17Evkc)로 확정했다 — `DECISIONS.md` 「폼 화면 실물 확정 7건」, `SYSTEM.md` §6-3. C-2 손익은 원장 초안이 탈락하고 대시보드로 다시 만들어 확정했다(보드 https://claude.ai/artifact/5kQyygb3hLbiQpo38BNznr, `DECISIONS.md` 「손익 화면 = 대시보드」 6건, `SYSTEM.md` §6-4 다시 씀, `tokens.css` 토큰 3종). 남은 초안 하나(`system/external-cert.html`)는 **아직 사용자가 본 적 없다 — 아무것도 확정되지 않았다.** 스크린샷은 찍어 두었고 어긋난 곳은 고쳤다(2026-09-18).

### 다음 세션이 할 일 (이 순서로, 한 화면씩)
1. 스크린샷은 `shots/cert-*.png`에 이미 있다(어긋난 곳 고침). 다시 찍으려면 `node docs/design/system/shoot.mjs cert-empty cert-sign cert-error cert-done cert-expired`
2. **C-3 외부 수령자 보드** → 「이대로 확정?」 → (a)~(c)를 한 건씩 → DECISIONS·SYSTEM 갱신
3. 끝나면 커밋·PR, 그다음 B(Pretendard)
4. C-2에서 배운 것: 사용자가 「대시보드 형식으로 한눈에」라고 하면 표를 더 얹지 말고 **컴포넌트 여러 종으로 실물을 다시 만든다**. 색은 「색 하나 = 뜻 하나」로 시작한다(6단 팔레트는 「정신없다」로 탈락). 카드 크기와 차트 크기는 맞춘다
