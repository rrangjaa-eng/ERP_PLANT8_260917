...
## 회차 4 — 전체 Codex 2차(codex-final2-A/B/C.md) 판정 (오케스트레이터)
- A-FF01 → 막는 문제 아님(선행 문제): domain/seed/index.ts:165-179가 INFO_ITEMS 전부를 매 시드마다 upsert해 노출 설정을 되돌리는 것은 기존 동작이고 Phase 4 플랜 04-20의 몫이다. 01:310은 insertVisibilityIfAbsent가 이를 막는다고 주장하지 않도록 문구만 고친다(04-20 의존 명시).
- 막는 문제 인정: B-RF01(폰 시트·PC 결재함 줄도 서버 가능 행동 목록을 그린다) · B-RF02(잔고 저장소 호출 수 단언을 실제 호출 구조와 맞춤) · C-F2-01(퇴직자 섹션: 잔고와 연차 조정 대상 연도는 같은 「섹션 연도」 하나에서 나온다)
- 경고 반영: A-FF02 · B-RF03 · C-F2-02 · C-F2-03 · C-F2-04
