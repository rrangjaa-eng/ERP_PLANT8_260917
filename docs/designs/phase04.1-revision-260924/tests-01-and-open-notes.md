...
## 멈춘 곳 (2026-09-25, Codex 끝 — 기술 리뷰 세션으로 인계)
- Codex: 1회차 3갈래(막는 6) → 2회차(24건 해결, 새 2) → 전체 1차(3) → 전체 2차(3 인정 · 1건 Phase 4 04-20 몫) → 전체 3차(A 0 · B 0 · C 1) → 바뀐 B·C 4차 **0 · 0**. 원문 codex-*.md, 판정 round2-brief.md 끝
- 범위 밖으로 남긴 것: B-F04(설정 화면 보기 전용 편집 컨트롤 — 기존 화면 전체 선행 문제) · A-FF01(시드가 노출 설정 되돌림 — Phase 4 04-20)
- /plan-design-review 때 UI-SPEC에 반영할 것: S2 approval.value 숨김 시 계급 이름만(없으면 `N단`) · S3 기안자=담당 → 승인+회수 · S8 `조직 범위가 특정 부서일 때만` 도움말 삭제 · S1 보기 전용 빈 화면 행동 없음 · S9 `{Y-1} 회계연도`/`올해 보기` 링크 · 월차 옵션 숨김 + 한 줄 · 보기 전용 관리자 조정 표만 · A-F03/C-F03 옛 D3/D4 문구 · B-F05 삭제 대상 도움말
- 다음: /plan-eng-review 04.1 (새 세션, CEO 리뷰 생략)
