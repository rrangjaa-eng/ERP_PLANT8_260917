...
## E. 2026-09-17 클라우드 실측 결과와 적용한 수정

클라우드 세션(리포가 `/home/user/ERP_PLANT8_260917`에 복제됨)에서 확인하고 고친 것:

- **GSD 절대경로**: `--local` 설치기가 `.claude/commands`·`agents`·`gsd-core`에 `F:/CLAUDE/ERP_PLANT8_260917/...` 절대경로 707곳을 써 넣어, 클라우드에서 `/gsd-*` 명령은 뜨지만 `@`-include 본문이 비어 있었다. 명령·에이전트 파일의 `@` 참조는 프로젝트 루트 기준으로 풀리는 것을 실측했다(루트 기준 `@.claude/...` 읽힘, `@~/...` 읽힘, 명령 파일 옆 상대경로는 안 읽힘). 접두사를 지워 `@.claude/gsd-core/...`로 바꿨다. `/gsd-update`는 그 PC의 절대경로를 다시 쓰므로, 업데이트 뒤 `scripts/gsd-relativize.sh`를 실행하고 커밋한다.
- **실행 비트**: Windows에서 커밋돼 `.claude/skills/gstack/bin/*` 88개와 shebang 스크립트 전부가 100644였다. 클라우드에서 gstack preamble이 "Permission denied"로 degraded 모드였다. `chmod +x` 뒤 커밋(내용 변경 없음). Windows 체크아웃은 `core.filemode=false`라 영향 없다.
- **gstack 번들**: SessionStart 훅이 `bun install` 뒤 `browse`·`find-browse`·`design`·`pdf` 네 번들을 직접 컴파일한다(약 3초, 각 100MB, gitignored). `bun run build` 전체는 벤더링에서 제외된 `bin/gstack-global-discover.ts`와 cso 소스가 없어 실패한다. 환경 변수 `GSTACK_SKIP_BUILD=1`로 생략할 수 있다.
- **Chromium**: gstack의 playwright 1.62는 chromium_headless_shell-1234를 찾지만 클라우드 VM에는 1194(Chromium 141)만 있고 Playwright CDN은 프록시를 통과하지 못한다. 훅이 `$PLAYWRIGHT_BROWSERS_PATH` 아래에 기대 리비전 → 설치된 headless_shell 심볼릭 링크를 만든다. `goto`·`text`·`screenshot` 동작 확인. headless 경로는 `GSTACK_CHROMIUM_PATH`를 읽지 않으므로 그 변수로는 해결되지 않는다.
- **gstack 벤더링 경고**: gstack 1.87은 벤더링을 deprecated로 표시하고 첫 스킬 실행 때 "team mode로 옮길까"를 한 번 묻는다. team mode는 개발자별 `~/.claude/skills/gstack`에 의존해 클라우드에서 안 되므로(§C) "B) 직접 관리"를 고른다.
- **superpowers**: 플러그인은 세션 시작 때만 설치되므로 `.claude/settings.json`이 있는 브랜치에서 새 세션을 열어야 한다. 이번 세션은 `master`에서 시작돼 미검증.
- **확인 방법**: 새 클라우드 세션에서 `/gsd-help`가 명령 목록을 출력하고, `/gstack` preamble 출력에 `SKILL_START_PROTO: 1`과 `VENDORED_GSTACK: yes`가 보이면 정상이다.
