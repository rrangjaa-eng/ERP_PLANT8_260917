...
smoke() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    run curl -fsS --retry 6 --retry-delay 5 --retry-all-errors "${target}/healthz"
  ...
  if ! run curl -fsS --retry 6 --retry-delay 5 --retry-all-errors "${target}/healthz" | grep -q '"ok":true'; then
