...
describe("domain/quotes/lines saveQuoteLines — 배치 충돌·전부 거부(04-04)", () => {
  it("(a) 읽은 버전이 서버 버전과 같은 줄 셋을 저장하면 셋 다 저장되고 버전이 1씩 오른다", async () => {
    ...
    const result = await saveQuoteLines(SYSTEM_VIEWER, revision.id, [
      { subcategory: subcategoryValue, itemName: "줄1", unitPrice: { currency: "KRW", amount: 100, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
      { subcategory: subcategoryValue, itemName: "줄2", unitPrice: { currency: "KRW", amount: 200, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
      { subcategory: subcategoryValue, itemName: "줄3", unitPrice: { currency: "KRW", amount: 300, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
    ]);
  ...
  it("(b)(c) 셋 중 한 줄의 서버 버전이 그 사이 올라가면 실제로 달라진 셀만 충돌로 담기고 세 줄 모두 DB에 반영되지 않는다 — 값이 같은 셀은 충돌이 아니다", async () => {
    ...
    const created = await saveQuoteLines(SYSTEM_VIEWER, revision.id, [
      ...
      { subcategory: subcategoryValue, itemName: "B", unitPrice: { currency: "KRW", amount: 2_000_000, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
      { subcategory: subcategoryValue, itemName: "C", unitPrice: { currency: "KRW", amount: 3_000_000, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
    ]);
  ...
  it("(d) 한 줄에 형식 오류(quantity <= 0)가 있으면 나머지 줄도 저장되지 않고 응답이 오류 좌표를 담는다", async () => {
    ...
    const attempt = saveQuoteLines(SYSTEM_VIEWER, revision.id, [
      { subcategory: subcategoryValue, itemName: "정상 줄", unitPrice: { currency: "KRW", amount: 100, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
      { subcategory: subcategoryValue, itemName: "형식 오류 줄", quantity: -1, unitPrice: { currency: "KRW", amount: 100, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
    ]);
  ...
  it("(f) 저장 성공은 행동 로그 1행, 거부는 행동 로그 0행을 남긴다", async () => {
    ...
    await saveQuoteLines(SYSTEM_VIEWER, revision.id, [
      { subcategory: subcategoryValue, itemName: "성공 줄", unitPrice: { currency: "KRW", amount: 100, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
    ]);
    expect(await countActionLogRows(revision.id)).toBe(before + 1);

    const afterSuccess = await countActionLogRows(revision.id);
    await saveQuoteLines(SYSTEM_VIEWER, revision.id, [
      { subcategory: subcategoryValue, itemName: "거부될 줄", quantity: 0, unitPrice: { currency: "KRW", amount: 100, fxRate: 1 }, execution: { currency: "KRW", amount: 0, fxRate: 1 } },
    ]).catch(() => undefined);
