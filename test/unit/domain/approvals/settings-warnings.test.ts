...
function orgUnitRow(id: string) {
  return { id, name: "경영관리본부", sortOrder: 1, archivedAt: null, createdAt: new Date(), updatedAt: new Date() };
}
...
async function warningsFor(values: Record<string, unknown>, kinds: DocumentKindDef[] = [kind("leave", true)]) {
  ...
  return listApprovalRouteSettingWarnings(viewer, {
    ...
    listOrgUnits: vi.fn().mockResolvedValue([orgUnitRow(MGMT_ID)]),
    findSimpleValues,
  });
}
